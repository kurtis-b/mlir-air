...
| `runlist/op.py` | 1566 | REWRITE | **`[2026-08-05]` 12 kernels, 16 runlist entries** (encoder; 13/17 decoder), read from `pattern/runlist/op.py:851-940`. Earlier drafts of this plan said 29 kernels / 42 entries, which is wrong and misled E4. Depends on Phase B. |
| `hybrid/op.py` | 709 | REWRITE | **`[2026-08-05]` 5 kernels, 5 runlist entries** (encoder; 6/7 decoder), from `pattern/hybrid/op.py:519-584`. The "12" earlier drafts quoted is the sum of both workload variants, not one layer. Renamed `coarse` — convention rule 7. **`[2026-08-05]` Largely already built**: `transformer_layer/builders/block.py` is a fused-operator sequence over one runlist, which is this mode. Phase E gives it a strategy directory and the shared instrumentation rather than writing it again. Note its measured shape is 4 sequences, and the two normalization points are 64 dispatches each, not one. |
