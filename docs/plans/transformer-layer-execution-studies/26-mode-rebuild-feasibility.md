...
| **N instruction streams under one xclbin** for `offload` | `python/air/backend/xrt.py` + `programming_examples/` | **own phase** | Already plumbed (`xclbin_input`, `xrt.py:80`, `:372-374`) but never dispatched; touches the shared `dispatch.py` path. **`[2026-08-09]` LANDED** (`93e15a64`, [29](29-offload-n-streams.md)) — five shapes in one xclbin, `context_loads 1` against 30, dispatch vector unchanged, E1 gate green. This row's sizing was right, with one addition it did not anticipate: **THREE distinct identifiers per stream are required and no caller set any.** `kernel_name` (a duplicate makes xclbinutil refuse the merge — the only loud one), `instance_name` (the loader matches kernels by substring) and `kernel_id` (the merged `AIE_PARTITION` routes to a PDI by `dpu_kernel_ids`; every AIR compile defaults to `0x901`). Collide the id and the second kernel times out at one shape and returns garbage at `mean_rel_L1` 1.41 with no error at the other. `agents/probes/probe_one_xclbin_n_streams.py` |
