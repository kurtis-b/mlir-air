...
| **N instruction streams under one xclbin** for `offload` | `python/air/backend/xrt.py` + `programming_examples/` | **own phase** | Already plumbed (`xclbin_input`, `xrt.py:80`, `:372-374`) but never dispatched; touches the shared `dispatch.py` path |
