...
| **N instruction streams under one xclbin** for `offload` | `python/air/backend/xrt.py` + `programming_examples/` | **own phase** | Already plumbed (`xclbin_input`, `xrt.py:80`, `:372-374`) but never dispatched; touches the shared `dispatch.py` path. **`[2026-08-09]` The mechanism is now DEMONSTRATED on hardware** — two GEMMs of different shape, one xclbin, one `hw_context`, both correct in either compile order. This row's sizing stands, with one addition it did not anticipate: **two distinct identifiers per stream are required and no caller sets either.** `instance_name` (the loader matches kernels by substring) and `kernel_id` (the merged `AIE_PARTITION` routes to a PDI by `dpu_kernel_ids`; every AIR compile defaults to `0x901`). Collide them and the second kernel times out at one shape and returns garbage at `mean_rel_L1` 1.41 with no error at the other. `agents/probes/probe_one_xclbin_n_streams.py` |
