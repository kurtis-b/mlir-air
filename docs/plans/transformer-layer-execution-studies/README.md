...
| study host suite | **652 / 652 in 30 modules**, pinned by `run_study_host_tests.lit` | 103 → 231 in 17 → 357 in 19 → 429 in 21 → 517 in 23 → 539 in 24 → 563 in 25 → 567 (devq 401) → 587 in 25 (FileCheck-verified red at 567, 581, 584, 585) → 600 → 603 → 607 → 610 → 611 (cluster B's review rounds) → 615 (the clock) → 652 in 30 (item 13: schema v3, `test_model_adapter`, `test_model_profiles`, `test_run_model`) |
...
| Item 13 — doc 56 H1a: model adapter + runner, schema v3, the kernel-scaling curve | done 2026-08-23 (devq 568 / 570 compiles, 569 smoke, 571 / 572 walks; `results/h1a-20260823/`) | **Schema v3** (13 model-scope columns appended last; `resume.row_key` + ledger `model_key`; `compare_roots` keys on the six, gates `tokens_per_second`; v2 roots read through `read_rows_compatible`); host suite 615 → **652 / 652 in 30 modules**; planner at the items-11/12 kernels (`gemv2`, derived 10-launch Qwen head; PLAN 10/10) and **every plan stage's launch count equals the cached manifests** for both models and phases. Walks 571 / 572 `complete: True`, `compare_roots OK` (drift med 2.6 % qwen, 1.3 % llama), verify PASS on every artifact set, Turbo before and after. **Qwen3-0.6B**: prefill M 2048 **1359 / 1395 tok/s** (1184 ms device of 1507), M 1024 **1371 / 1369** (O+FFN forced fused-cast — the cascade is fused-cast-only and the registry best at 512 / 1024 is drain: `'%arg15' ... 'memref<512x1024xbf16>' vs 'memref<512x1024xf32>'`, devq 568), M 512 skipped (gate/up fused-cast row is `tile_n 96`, one `mm.o` per ELF, devq 570); decode ctx 512 / 1024 / 2048 **11.85 / 10.11 / 7.07** and 12.19 / 10.23 / 7.29 tok/s — device flat 63–66 ms, host attention 12.5 → 24.2 → 58.7 ms. **Llama-3.2-1B**: prefill **1750 / 1736 tok/s**; decode **10.70 / 10.16 / 8.90** and 10.88 / 10.28 / 8.77 (device 76–77 ms, host 8.1 → 25.0). Per-token vectors 57 subs / 150 launches (qwen), 33 / 152 (llama) = the plan's. Found, not fixed: `--run-only` leaves the QKV scratch layout unset (17 args to an 18-arg ELF at M 2048) | [56 §4](56-full-model-mixed-precision-study-plan.md) |
...
| 13 | ~~**6b** — doc 56 H1a: model adapter + runner, schema v3, the kernel-scaling curve keyed by `Plan.sha`~~ **done 2026-08-23** (devq 568–572) — `llms/shared/model_adapter.py`, `study/run_model.py` + `model_profiles.py`, schema v3 (host suite 652/652 in 30); two walks `complete: True`, `compare_roots OK`, verify PASS per artifact set; Qwen3-0.6B decode 11.9 / 10.1 / 7.1 tok/s at ctx 512 / 1024 / 2048, Llama-1B 10.7 / 10.2 / 8.9, prefill 1359–1395 and 1736–1750 tok/s at M 2048; the curve's M = 1024 point (1371) runs the O+FFN cascade **forced fused-cast** and M = 512 hit a second wall (the registry's `tile_n 96` gate/up row vs one `mm.o` per ELF) — both walls named in [56 §4](56-full-model-mixed-precision-study-plan.md); a drain-capable cascade is the repair | done |
