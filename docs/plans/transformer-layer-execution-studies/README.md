...
`[2026-08-12]` **The open rows are 6c, 8, 12, 13 and 14, and they are not sequenced** — 6c
...
**`[2026-08-12]` Five rows closed that day and two are new.** **9** (the silent multi-get memref
shrink, fixed with a real extent computation) and **10** (the column census, which now counts
per-column MM2S *demand* over both flow kinds and over packet flows, with its negative control
running inside the gate on every invocation) both landed. **11** closed as DEFER — 11(a) turned out
to be 11(b)'s prerequisite rather than its sibling. **12**'s three options were each scoped
...
| **12** | ~~**Phase G and the two goals — never started, and never chosen.**~~ **`[2026-08-12]` ALL THREE INVESTIGATED — the choice is now informed, and it is still yours.** Each has a scoping doc, and each investigation found that its own spec overstates or misdirects the work. **Phase G** ([34](34-phase-g-scoping.md)): doc 10's spec is substantially obsolete — `devq.sh` already replaced its whole serialization tier and pmode enforcement here *refuses* where iron warns; the gate sentence is satisfiable by existing scripts today. Its real blocker is **reachability** — one of six declared families is reachable (`run_mode.py` hardcodes `encoder_bert`, every SPECS row is `emb_dim 768`), so a "full profile" needs a Phase-C-sized sweep first. Recommends dropping four of doc 10's behaviours on measured grounds. **Goal 1** ([35](35-goal-1-sliding-window-scoping.md)): the kernel change is two branches in ONE function shared by all ten models, but a window is **correctness-only with zero speedup** (masking is element-wise, not tile-skipping), the speedup route is a hang path plus queue items 8 and 9 verbatim, and **the specified gate may be vacuous** — full causal is a superset of the window, so a degraded implementation would often still pass. Weights are absent and license-gated. **Goal 2** ([36](36-goal-2-quantized-scoping.md)): `make verify` DOES exercise the quantized path (31 of 32 gated tokens are int4 decode), and doc 12 **names the wrong second model** — hard shape asserts fail Llama-3.2-3B and leave SmolLM2-1.7B, which doc 12 never mentions, as the shape-exact candidate; the top risk is that a compatible checkpoint may not exist, checkable in an hour. **Still a decision, not a blocker** — but now a decision with three costed options and two spun-out items (13, 14) | A phase each; pick one | [34](34-phase-g-scoping.md) · [35](35-goal-1-sliding-window-scoping.md) · [36](36-goal-2-quantized-scoping.md) |
| **13** | **Two latency-asserting gates have NO pmode guard** `[2026-08-12]`, found while scoping Phase G ([34](34-phase-g-scoping.md)). `require_turbo` is wired into exactly three files — `run_mode.py`, `component_groups.py`, `sweep/registry_sweep.py` (which *refuses*, stricter than iron's warn-and-continue) — and **`gate-h.sh` leg 4 is not one of them**, though it asserts decode throughput against a recorded 9.435 tok/s floor; `run_npu2_runlist_gate.lit`'s zero-margin latency clause is the same class. Verified at merge: no `pmode`/`turbo` match anywhere in `gate-h.sh`. At `Default` the measurement runs ~15–20× slow, so the gate fails and **reads as a compiler regression** — trap 0's exact failure mode, reaching the one place that halts an unattended run. The floor file records sha, timestamp and prompt but **not the pmode it was measured at**, so the floor itself is unconditioned | Small: guard both, and stamp pmode into the floor file | [34](34-phase-g-scoping.md) · trap 0 |
| **14** | **The int4 model's verify lit is DISABLED, uniquely among the ten** `[2026-08-12]`, found while scoping Goal 2 ([36](36-goal-2-quantized-scoping.md)). `llms/llama32_1b_int4/run_npu2_verify.lit` is `// REQUIRES: false`; verified at merge against all ten — every other model carries a real `ryzen_ai_npu2, peano, hfweights_*` line. Disabled upstream in `18d1dac2` for **CI-runner OOM**, not an int4 defect. Consequence: **the only quantized gate in the tree runs solely via the manual ten-model loop and never in lit**, so any lit-based CI (queue item 12's Phase G) tests zero quantized code. Not urgent — the ten-model loop does gate it — but it is a hole that a CI phase would silently inherit | Small to diagnose; the fix depends on whether the OOM still reproduces | [36](36-goal-2-quantized-scoping.md) |
...
3. **Then pick from the queue above.** The open set is small and none of it is sequenced:
   ...
   **8** is the one unclaimed compiler defect left — a `tileChannelOpByFactor` abort on the
   two-symbol maps doc 23's own L3-side rule produces routinely, at **three** source sites;
   ~~**9** and **10**~~ **9 and 10 closed 2026-08-12** (the silent shrink now computes a real
   extent; the column census counts demand and carries its own negative control) — but read 9's
   row before touching R2, because **its fix invalidates 31b's reason for avoiding literal-offset
   L1 bands**; ~~**11** is Phase F's remainder~~
