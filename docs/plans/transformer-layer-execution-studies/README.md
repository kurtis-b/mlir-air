...
the **operator's `install-xrt` refresh**, then item 6's gate run. (3) **Item 7 CLOSED
...
operator's `install-xrt` refresh); **item 7 CLOSED** (two-pass `addnorm` landed, suite green
with the new kernels, offset rows measured at `atol_required` 0.0, provenance refreshed —
[23 §2](23-rules-and-open-items.md)). ~~Item 1(c)'s ten-model re-run~~
