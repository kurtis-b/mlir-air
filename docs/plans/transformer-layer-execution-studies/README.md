...
blocks only the resident tail, and everything else is independent of it and of each other.
(**14 closed later that day**: the int4 verify lit's OOM does not reproduce here, it is re-enabled
and passing — devq 255.) The
...
3. **Then pick from the queue above.** The open set is small and none of it is sequenced:
   ...
   **12** is Phase G and the two goals — **all three now scoped, none chosen**, which makes it the
   ...
   disabled verify lit~~ **14 closed the same day** — the OOM it was disabled for does not
   reproduce on a host of the size the disable named (10.53 GiB peak split / 12.57 GiB
   single-process, devq 255), and the lit is re-enabled and green, so lit CI now carries the
   tree's only quantized end-to-end gate; **15** is the third unguarded latency gate,
   `compare_roots.py`, which
