...
decision: `AIR_OFFLOAD_LEGACY_ELF=1` is now the ELF opt-in, no pinned literal moved. What
remains is the four-mode re-walk the flip forces — the first *un*-conditional cost-decomposed
comparison, with Turbo a checked precondition, since every recorded `offload` number predates
the flip and describes the ELF path. (2) **Item 6a, the `air-fuse-channels` crash fix** (trap 4): the resident tail's
...
fronts, none blocking the others: **item 3's remainder** (the four-mode re-walk — the flip
itself landed 2026-08-11; the first unconditional cost-decomposed comparison); **item 6a**
