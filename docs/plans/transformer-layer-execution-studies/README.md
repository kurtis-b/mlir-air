...
`[2026-08-12]` **The open rows are 6c, 12, 21 and 22, and they are not sequenced** — 6c
...
| **22** | **`mmult_aie2{,p}.py` parse `--herd-m`/`--herd-n` and never read them — so a benchmark's printed latency is for a shape nobody requested** `[2026-08-12]`, found while closing the cleanup batch. `mmult_runner(air_ir_string=...)` falls back to the module defaults, so **asking for a herd shape silently gets the default one** and the number that comes out is attributed to the requested shape. This is item 19's `--compile-mode` verbatim — a flag parsed and never branched on — but in a *benchmark* rather than a dispatch guard, which makes its output wrong rather than its behaviour unsafe. **Any recorded `mmult` figure taken with a non-default `--herd-*` describes the default shape**; whether any such figure is cited anywhere has not been checked. Left outside the cleanup batch deliberately (different directory, different owner) | Small: read the flags, or refuse them; then check whether anything cites an affected number | [23 §3](23-rules-and-open-items.md) |
