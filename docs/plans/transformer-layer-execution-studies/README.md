...
| `offload` | **Half corrected and gated** 2026-08-08. The LINEARITY half is done: both attention matmuls on device, only softmax / both LayerNorms / GeLU on the host. **`[2026-08-09]` It currently pays the MAXIMUM reconfiguration cost, not the minimum** — `_evict_context` (`offload.py:465`) reloads the `hw_context` before every dispatch, 30 times per layer. That eviction is measured, but its **scope is narrower than its docstring claims**: the corruption is `elf`+`[2,2]` specifically, and `xclbin`+`[2,2]` reuses cleanly (`probe_context_reuse.py`). So the 30 reloads are removable, and they are the leading candidate for this mode's 120% intra-walk latency drift ([27](27-common-ladder-result.md)) | **N instruction streams under one xclbin** — untouched, and it is where the mode's reconfiguration-minimizing claim actually gets tested. Already plumbed (`xclbin_input`, `xrt.py:80`, `:372-374`), never dispatched. Touches `llms/shared/infra/dispatch.py:685`, shared with `fused`. **`[2026-08-09]` FEASIBLE, and demonstrated end to end on hardware.** Both halves of the question are answered favourably. Context reuse is fine — under the xclbin ABI at the mode's existing `[2,2]` a kernel re-executes bit-identically (`probe_context_reuse.py`). And **two GEMMs of different shape chained with `--xclbin-input` both execute correctly from ONE `hw_context`**, in either compile order (`probe_one_xclbin_n_streams.py`). **It requires TWO distinct identifiers per stream, and no caller in this tree sets either:** a distinct `instance_name` (the loader matches kernels by *substring*, `xrt.py:634`) **and** a distinct `kernel_id` (the merged `AIE_PARTITION` routes a kernel to its PDI by `dpu_kernel_ids`, and every AIR compile defaults to `0x901`). With ids colliding the second kernel times out at one shape and returns **garbage at `mean_rel_L1` 1.41 with no error raised** at the other. `pattern/offload` today names every drain GEMM `matmul_bf16` and sets no kernel id, so at 1024 all five of its shapes would collide on both axes at once. The remaining work is the dispatch layer as originally scoped: `plan_submissions` keyed on array-configuration identity (`artifact_of` as the default proxy), `ensure_loaded` sharing one context across artifacts that share an xclbin, and per-shape identifiers in `offload_config` |
...
| Can N instruction streams share one xclbin? | **Yes, demonstrated on hardware** — two GEMMs of different shape chained with `--xclbin-input`, both correct from ONE `hw_context`, in either compile order. **It needs two distinct identifiers per stream and no caller sets either:** `instance_name` (the loader matches by substring) and `kernel_id` (the merged `AIE_PARTITION` routes to a PDI by `dpu_kernel_ids`; every AIR compile defaults to `0x901`). Collide the ids and the second kernel times out at one shape and returns garbage at `mean_rel_L1` 1.41 **with no error** at the other — verified as a both-directions control. The instruction streams are a red herring: the second kernel's `insts.bin` is byte-identical to its standalone build either way, so the discriminating field is inside `AIE_PARTITION`. | `agents/probes/probe_one_xclbin_n_streams.py` |
