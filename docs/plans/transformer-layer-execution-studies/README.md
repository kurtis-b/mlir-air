...
| study host suite | **656 / 656 in 30 modules**, pinned by `run_study_host_tests.lit` | 103 → 231 in 17 → 357 in 19 → 429 in 21 → 517 in 23 → 539 in 24 → 563 in 25 → 567 (devq 401) → 587 in 25 (FileCheck-verified red at 567, 581, 584, 585) → 600 → 603 → 607 → 610 → 611 (cluster B's review rounds) → 615 (the clock) → 652 in 30 (item 13: schema v3, `test_model_adapter`, `test_model_profiles`, `test_run_model`) → 656 (item 13's review round: recorded driver traces, the gate's binding clauses) |
