...
`[2026-08-12]` **The open rows are 6c, 8, 9, 11 and 12, and they are not sequenced** — 6c
blocks only the resident tail, and everything else is independent of it and of each other.
**Item 10 closed the same day** (the R1 column census counts the rule and carries its own negative
control). The struck rows are kept because their evidence chains are cited elsewhere; read the bold
verdict, not the whole cell.
...
3. **Then pick from the queue above.** The open set is small and none of it is sequenced:
   ...
   **8 and 9** are unclaimed and independent — two compiler defects that will ambush a later
