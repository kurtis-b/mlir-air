...
Suite **30/30 on NPU2**, all ten shipped models verify (both re-run 2026-08-10).
...
ranks *implementations*, not the four modes; latency dated 2026-08-10 is conditional on the
machine anomaly; and [27](27-common-ladder-result.md)'s warm DRAM ordering is superseded by
[32](32-cost-decomposed-ladder.md)'s (the targeted pool eviction moved `runlist` below `fused`
and `coarse`).

**The front of the queue is item 3: the `offload` default flip, now decidable at the canonical
shape.** The 4096 wall fell completely on 2026-08-11, in three acts in one day: the multi-launch
packaging compiled (from `623768f2` + install activation), its dispatch experiment answered NO
(**in-stream `load_pdi` faults the firmware**, `fatal_error_type 0x10`; the scoped
`--expand-load-pdis` fallback proved a no-op on this toolchain and was falsified in doc 29), and
route 3 landed instead — the shared chain pins a `fused-cast` winner to the shape's `drain` row,
runs **10/10 clean at 4096 with `context_loads 1` for 30 dispatches**, and
`run_npu2_offload_peano.lit` now gates all three recipes at 4096. Also owed: item 1(c), the
suite + ten-model re-run under the new install. After the flip: the four-mode re-walk and the
first *un*-conditional cost-decomposed comparison — genuinely unconditional now, since the pmode
anomaly (item 1b) is resolved and Turbo is a checked precondition.
...
| `offload` | **Corrected and gated on BOTH halves** as of 2026-08-09. The LINEARITY half (2026-08-08): both attention matmuls on device, only softmax / both LayerNorms / GeLU on the host. The RECONFIGURATION half (`93e15a64`, [29](29-offload-n-streams.md)): five GEMM shapes in **one xclbin**, `context_loads 1` against the ELF path's 30, dispatch vector unchanged by design. `_evict_context`'s 30 reloads were the maximum reconfiguration cost in the mode defined to minimize it | **`[2026-08-11]` The 4096 compile wall is DOWN and the backend is ACTIVE** — multi-launch xclbin packaging (`623768f2`, queue item 1) builds the five-shape chain at 4096, activated in `install-xrt` by the operator with the fixture re-verified. Two things left. **The dispatch experiment**: no multi-launch xclbin has ever run on hardware — in-stream `load_pdi` is the open question, fallback scoped. And **the default is still ELF** (max reconfiguration, `context_loads` 30/dispatch now a column): flip after 4096 dispatches, per the recorded decision — which re-dates every recorded `offload` number |
| `fused` | **Builds and gates again** 2026-08-08, at **1024** — it was red and unrunnable, pinned at a 4096 its own builder rejects. `[2026-08-10]` Its definitional gap (packaged, not resident) is now a **specced phase with a measured prize**: 84.0 → 16.5 MiB of DRAM crossings at 1024, the J7a+J7b composition already routing within the column budget hermetically ([31](31-fused-resident-tail.md), queue item 6) | One xclbin stays blocked for the *measured* reasons (the `[1,1]`/`[2,2]` conflict plus `air-fuse-channels`) — but the TAIL scope avoids both, which is what makes the resident-tail phase buildable. Whole-tensor residency stays capacity-bounded; streaming residency is not (per-stage L1 fits are seq-independent, [31a](31a-resident-byte-floor.md)) |
