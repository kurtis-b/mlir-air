...
| # | Work | Size | Spec |
|---|---|---|---|
| **1** | **Corrected `coarse`.** Build cells **C2** (`block` front, decomposed tail) and **C3** (`runlist` front, banded tail) and measure at **2048/4096** — never 1024, where every cell is dominated by one that already has a mode name. First step is mechanical: extract `prepare_runlist`'s and `prepare_fused`'s nested dispatch closures (`runlist.py:597-834`, `fused.py:593`) to module scope **with no behaviour change**, proven by re-running both modes' lit recipes | Large — new builders, new gate, new measurement | [28](28-coarse-blend-space.md) |
