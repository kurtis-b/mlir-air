...
| study host suite | **685 / 685 in 31 modules**, pinned by `run_study_host_tests.lit` | 103 → 231 in 17 → 357 in 19 → 429 in 21 → 517 in 23 → 539 in 24 → 563 in 25 → 567 (devq 401) → 587 in 25 (FileCheck-verified red at 567, 581, 584, 585) → 600 → 603 → 607 → 610 → 611 (cluster B's review rounds) → 615 (the clock) → 652 in 30 (item 13: schema v3, `test_model_adapter`, `test_model_profiles`, `test_run_model`) → 656 (item 13's review round: recorded driver traces, the gate's binding clauses) → 660 → 662 (item 14 + review) → 679 → 681 in 31 (item 16 + review) → 685 (item 17: the int4 binding, w4-decode profile, quant columns) |
| seam lit | **40 + 12** (`dispatch` 40/40 since item 14's review, `plan` 12/12 since item 17) | 35 + 10 → 40 + 11 (items 14 / 16) → 40 + 12 |
...
| O1 second half — QKV at 2 launches (in-core QK-norm + RoPE) | landed 2026-08-23 (devq 553–558) | new kernel `mv_heads.cc`; stage 0.672 → 0.494 ms/layer (89 µs per boundary); verify 2/0; reexec 7/7; profile 13.04 / 12.91 → 13.40 / 13.52 tok/s, `rms_qkv` 0.65 → 0.49; 206 → 150 boundaries/token; q/k closer to f32 than the 4-launch form, not bit-identical | [57 §5 item 5c](57-inference-path-optimizations-from-hexagon.md) |
| Llama-1B head: mixed partition vs `m_input 8` | landed 2026-08-23 (devq 563/564) | four cells ×20: 8 × 16384 mi4 15.10 / 7 × 16384 + 13568 mi4 15.15 / mixed mi8 14.16 / 8 × 16384 mi8 13.91 ms — pad rows never pay, `m_input 8` −1.19; shipped `m_input 8`, uniform partitions: `lm_head_gemv` 14.94 → 13.86–13.98 ms, verify 2/0, reexec 7/7 (gate body shared in `shared/infra/lm_head_reexec.py`) | [57 §5 item 5d](57-inference-path-optimizations-from-hexagon.md) |
...
| Item 9 — R1 as iron's partial-sum-staging pipeline, ported and measured | done 2026-08-22 (devq 512–524; `results/r1-supertile-20260822/`) | `builders/tail_pipeline.py` runs AN1→FFN→AN2 resident at 512×768×3072 (PASS 3/3, 0/393,216, corr 0.999933; 320 re-executions on one context verified) after a builder-side deadlock fix (a put BD given two tokens by buffer-level lock pairing) and four AIR walls; band-serial (32 × 16-row dispatches), `n_b` capped at 2 by placement. device **26.02 ms vs `coarse`'s tail 4.66 ms (5.6×)**, wall 26.39 vs `coarse`'s whole forward 12.77 — band-serial re-reads the weights 32× per layer (302 MB) to save a 6.3 MB H round trip, as 31 §3.6 predicted. Two compiler defects reported. R1 closed on hardware | [31 §2.3](31-resident-tail-r1-record.md) |
| Item 10 — the re-execution family, mechanism named and fixed in the compiler | done 2026-08-22 (devq 527–538; `results/reexec-matrix-20260822/`) | **LOAD_PDI parity**: `expand-load-pdi` alternates two empty reset PDIs by position per dispatch, the NPU2 firmware skips a load whose id it loaded last, so an ODD count leaves the next dispatch's launch 0 unreset (7/7 rows; ×20 clean vs ×1 wrong on the same launch). `AIRRtToNpuPass` pads odd dispatches with one tile-less configure (26–32 µs, odd ELFs only); lit verified failing first; `check-air-mlir` 506/7/7; family clean ×5; production gates green | [57 §1.5](57-inference-path-optimizations-from-hexagon.md) · [16 §18](16-compiler-changes.md) |
...
| Item 13 — doc 56 H1a: model adapter + runner, schema v3, the kernel-scaling curve | done 2026-08-23 (devq 568 / 570 compiles, 569 / 574 smokes, 571 / 572 first-pass walks — superseded; devq 573 Codex review, five blocking findings fixed; **575 / 576 walks**; `results/h1a-20260823/`) | **Schema v3** (13 model-scope columns appended last; `resume.row_key` + ledger `model_key`; `compare_roots` keys on the six, gates `tokens_per_second`; v2 roots read through `read_rows_compatible`); host suite 615 → **656 / 656 in 30 modules**; planner at the items-11/12 kernels (`gemv2`, derived 10-launch Qwen head; a forced GEMM method is part of the plan and its hash, `h0.3`; PLAN 10/10) and **every plan stage's launch count equals the cached manifests** for both models and phases, checked live on every rung and pinned over recorded driver traces (`study/fixtures/h1a_driver_traces/`). **A `passed` row is bound to what it measured**: the gate loads the timed artifact set (ELF bytes sha256'd before timing, before and after the gate), runs the row's own prompt at its own length (a prefill rung's full M tokens, `max_seq = M + 32`), must exit 0 and write its own report. Walks 575 / 576 `complete: True`, `compare_roots OK` (drift med 1.3 % qwen, 1.1 % llama), verify PASS on every set, Turbo before and after, ~4 min per walk. **Qwen3-0.6B**: prefill M 2048 **1367 / 1371 tok/s** (1185 ms device of 1498), M 1024 **1359 / 1338** (O+FFN forced fused-cast — the cascade is fused-cast-only and the registry best at 512 / 1024 is drain: `'%arg15' ... 'memref<512x1024xbf16>' vs 'memref<512x1024xf32>'`, devq 568), M 512 skipped (gate/up fused-cast row is `tile_n 96`, one `mm.o` per ELF, devq 570); decode ctx 512 / 1024 / 2048 **12.01 / 10.62 / 7.31** and 12.17 / 10.33 / 7.35 tok/s — device flat 63–66 ms, host attention 11.6 → 21.7 → 55.6 ms. **Llama-3.2-1B**: prefill **1736 / 1739 tok/s**; decode **10.75 / 10.03 / 8.85** and 10.83 / 10.18 / 8.99 (device 76–77 ms, host 7.7 → 25.6). Per-token vectors 57 subs / 150 launches (qwen), 33 / 152 (llama) = the plan's. Found and fixed: `--run-only` left the QKV scratch layout unset (17 args to an 18-arg ELF at M 2048); `restore_scratch_layout` now owns it | [56 §4](56-full-model-mixed-precision-study-plan.md) |
| Item 14 — the drain-capable O+FFN cascade (doc 56 H1a walls) | done 2026-08-25 (devq 582 compiles; 584 production verify; **585 / 586 walks**; 589 re-execution gate PASS; `results/item14-20260825/`) | The cascade is **per-GEMM**: shared `o_ffn_multi._build_o_ffn` takes each GEMM's registry method + tile_n independently (drain 1 launch / fused-cast 2 + f32 scratch), per-GEMM `mm.o` variants co-linked in one ELF; new air-free layout owner `gemm_builder.o_ffn_gemm_layout` read by builder, compile and `restore_scratch_layout`/`_o_ffn_call`; `build_o_ffn_qwen_module` a thin delegate (the ~250-line copy gone); `gemm_method=` stays as the test-only override. M 2048 module byte-identical both models; recompiled M 2048 ELFs differ only in aircc `aie_image` metadata (untouched flash_attn shows the same drift — the control). Host suite 656 → **660/660 in 30**. Device: M 512/1024 compiled UNFORCED (`artifact_deviation: null`, o_ffn 8 launches = the plan's); `make verify` PASS on the recompiled production path; walks 7/8 `complete: True` 10/10, curve **3/3** — prefill M 512 **1270 / 1273 tok/s** (first M 512 point), M 1024 **1372 / 1353** on the drain plan (486 launches, plan source derived again; forced walks 3/4 read 1359 / 1338 at 598), M 2048 1376 / 1390; decode and llama in band; `compare_roots` OK (drift med 1.19 / 1.11 %), verify PASS per set on the timed bytes, item-15 host_ops check green. Deviation plumbing changed (661st host test): a loaded FORCED set restores its compile.json deviation's layout — the registry restore left the forced M 1024 ELF's f32 scratch args unbound (devq 583's nondeterministic verify FAIL, root-caused); gate ×3 on that set PASS after the fix (devq 590). Review round (devq 591, 1 blocking + 1 non-blocking, fixed): the ELF is the ABI ground truth — `dispatch.elf_arg_count` reads the arg count from the binary's .dynsym and `load_and_run` refuses any mismatch before any device work (seam 35 → 40/40, study 662/662); `artifact_content_sha` includes `artifact_deviation`; deliberately mismatched cache REFUSED on device (`ArgCountMismatchError ... called with 15 ... declares 19`, devq 592/594, control PASS), fresh walk 9 complete 10/10 curve 3/3 (1281/1346/1393 tok/s), verify PASS per set (devq 593) | [56 §4](56-full-model-mixed-precision-study-plan.md) |
| Item 16 — doc 56 H1b: valid ubatch prefill, the two-point ubatch curve | done 2026-08-25 (devq 595/596 probe, 597 compiles, 598/599 re-execution gate, **600 / 601 walks**; `results/item16-h1b-20260825/`) | **The §3.4 decision by probe**: rectangular head-first FA (compile-time `(Lq,Lk)` grid) over a run-time kv trip count — ~20 lines confined to the causal counter (boot + wrap at `(lk−lq)/64`; square IR byte-identical, sha-pinned); tiny (256,512) through full aircc in 7.5 s, no walls; runtime trips stay the recorded alternative for a finer grid. Landed: `run_npu_prefill_chunked` (chunk-outer / layer-inner, per-layer KV append before attention, RoPE at absolute positions with the LUT non-static, NO padding path), `npu_fa_headfirst_kv` + `flash_attn_ctx<Lk>` artifacts, the adapter's `prefill(ubatch_policy="chunked")` with per-chunk records, **`plan_ubatch_prefill`** (the chunked phase modelled: 169 subs / 171 host_ops / 962 launches at 2×512, enforced live), the `ubatch-curve` profile, and the gate through the SAME chunked path (`LLMS_VERIFY_UBATCH`; its 32-token decode continues from the incremental KV cache). Host suite 662 → **679/679 in 31**; seam PLAN 10 → 11. Re-execution gate: first run failed an arbitrary 0.999 f32-cos bar (rect 0.998987, deterministic); settled by measurement — the **composition identity is BIT-EXACT** (the rect kernel's rows == rows [512:) of the square-1024 device kernel, byte-for-byte) and dispatch 2 byte-equal on all shapes. The curve (same 1024-token prompt, both walks `complete: True`, verify PASS per set on the timed bytes, `compare_roots` OK, drift med 1.48 %): ubatch 512 **1179.9 / 1173.6 tok/s** (TTFT 0.85–0.89 s; 169 subs / 962 launches), ubatch 1024 **1334.5 / 1302.1** (TTFT 0.75–0.79 s; 85 / 486) — halving the ubatch costs ~11 %, attributed to +84 submissions / +476 boundaries, +59 MB KV re-read, and the rectangular FA's 2× keys per query. **Review round 2026-08-26** (devq 602, 3 blocking + 1 non-blocking, all fixed): the FA CLI's causal oracle offset-fixed + a rectangular lit, on device 0/65536 mismatches (devq 603); the re-exec gate asserts strictly — family-standard isclose 0 mismatches, dispatch-2 byte-equal, composition identity BIT-EXACT asserted, no cosine floors (devq 604); the chunked scheduler tested as CODE — fake-NPU host test runs the real `run_npu_prefill_chunked` vs `run_npu_prefill` byte-equal with a static-skip teeth check (host suite 679 → **681/681 in 31**), and on device the chunked path diverges LESS than a no-chunking M-2048 single-shot control at every layer (ratio ≤ 0.80, layer-0 byte-equal, top-1/top-5 equal for the pair under test; legs devq 607); the square-IR pin gains the production 1024 square. Kernels unchanged → one fresh walk: walk 3 (devq 606) complete, verify PASS, 1125.2 / 1345.3 tok/s, compare_roots walk1→walk3 OK (drift med 2.72 %) | [56 §4](56-full-model-mixed-precision-study-plan.md) |
| Item 17 — doc 56 H2a: the existing int4 decode decomposed under the runner | done 2026-08-26 (devq 608 re-execution check, **609 / 610 walks**; `results/item17-h2a-20260826/`, prediction file predates the first measure job by file time: 01:04 vs 01:16) | The EXISTING `llama32_1b_int4` driver bound as one `ModelBinding` row (`precision_plans=("w4_decode",)`, `model_path` Session kwarg, `quant_contract_module`), no fork; the verify adapter gains the loaded-cache hop (the gate runs the TIMED bytes); every planned host stage a named bucket (`kv_append` joins the int4 decode block and prefill loop); the `w4_decode` decode plan names the shipped sequence (`rms_qkv_int4_rope` 6 / `o_gemv_ffn_int4` 3 / `lm_head_gemv` 8 = **33 subs / 152 launches / 34 host ops per token**, checked live), each int4 GEMV naming the quant contract; `quant_*` columns POPULATED from `awq_repacker.quant_contract` (gs 128 by ast from the loader + the checkpoint id; accum pinned to `mv_int4_bf16.cc`'s accfloat; GEMM contract != GEMV contract on every row). Host suite 681 -> **685/685 in 31**; seam PLAN 11 -> **12/12**. Walks on the SHIPPED 08-19 caches, `complete: True` both, verify PASS 3/3 prompts on the loaded bytes, `compare_roots` OK (0 failures; walk 1 host-attention transient, max token 505.7 ms; `device_ms` drift 0.9 %): **walk 2 citable — 14.67 / 13.60 / 11.21 tok/s = 68.2 / 73.6 / 89.2 ms at ctx 512/1024/2048** (walk 1: 13.56/12.99/9.80). THE ATTRIBUTION: device ~57.0 ms ctx-flat = 25.7 weight-stream floor (1046.2 MB at 40.8 GB/s) + **16.3 boundaries** (152 x 107 us; the QKV line is ~90 % boundary) + **~15.5 dequant excess, nearly all in `o_gemv_ffn_int4`** (1.97 ms/call = 0.32 bnd + 0.70 stream + 0.95 dequant; 14.4-14.6 GB/s effective); host attention 7.5 -> 25.5 ms (= the bf16 sibling's) + 3-5 ms glue on top. vs bf16 llama: 1.35x/1.34x/1.26x, not the 4x byte ratio; the June '56 ms / 17.8 tok/s' header is pmode-unrecorded, artifact-free, and not reproducible under the study clock (nearest point 68.2 ms). Re-execution check on the shipped pre-fix bytes: parity rule **3/3 as predicted** (o_gemv 3 loads ODD -> wrong at d2/d3 back-to-back, healed by one other-ELF dispatch; evens clean; production alternation unaffected) — doc 57 §1.5's rule now 10/10 | [56 §4](56-full-model-mixed-precision-study-plan.md) |
| Item 18 — doc 56 H2b: `w4_decode` for Qwen3-0.6B | done 2026-08-26 (devq 612 compile, **621 strict re-execution SHIP gate** (614 first pass superseded, 618–620 the strict rewrite's own falsifications), **615 / 616 walks**; `results/item18-h2b-20260826/`, prediction 02:10 predates the first device job 02:30 by file time; review round devq 617, both blocking + both non-blocking findings fixed) | Priced per candidate from H2a's charge model, then built ONLY what pays: the O+FFN cascade goes int4 — the llama 3-launch cascade with `q_dim=` (O decoupled, M=emb 1024 / K=2048) and k_chunk=1024 (one `mv_int4_bf16.o` at DIM_K=1024, the .o tag now carries k_chunk; llama IR byte-identical, sha-pinned), NO new kernel, launch structure UNCHANGED (the w4 dispatch vector = the bf16 plan's 57 subs / 150 launches / 58 host ops, live-checked). int4 QKV and the int4 head NOT built — priced negatives recorded as the plan's rejected candidates (the 107 us boundary beats the QKV byte ceiling in any fallback structure; the in-core-epilogue form is a new-kernel project ≤2.3–4.5 ms; the head is O4's measured −0.46 ms). `w4_decode_pack.py` is the ONE owner (QWEN3_W4_DECODE default OFF — bf16 stays the operator default; RTN uint4 gs=128 pack + the dequantized-copy substitution so prefill/decode/oracle are ONE model), wired binding → prepare-env → driver → loader → `build_hf_model` patch → gate env; `compile-decode` + plan-aware decode-set discovery + the `w4-decode-qwen` profile. Host suite 685 → **691/691 in 32** (+1 review round: the llama default-IR golden); seam PLAN 12 → **13/13**. Gates: re-exec parity predicted first (3 loads ODD → the §18 pad engages) and CLEAN — devq 621 STRICT (review round): each stage per-element at its own family bound against its own device-side input, 0 mismatches everywhere (K_div=2/1/3 all certified), d2–d4 byte-equal on the production-contract outputs; walks `complete: True`, verify **PASS 3/3 both** on the LOADED timed bytes vs the dequant-patched HF oracle, `compare_roots` OK (tok/s drift 3.3 %). **THE VERDICT: w4_decode pays — 14.87/14.91, 12.00/12.40, 8.09/8.37 tok/s at ctx 512/1024/2048 (67.1–67.3 / 80.6–83.3 / 119.5–123.6 ms) vs bf16's 11.94–12.04 / 10.08–10.43 / 7.19–7.30: −13 to −20 ms/token, +19–24 % at ctx ≤1024, ALL SIX tokens inside the predicted bands; device 49.9–52.9 ms ctx-flat vs 62.9–65.6.** The miss, named: the cascade's non-boundary rate is 10.0 GB/s packed (the head probe's class, not llama's 19), dequant residual 12.8 ms/token vs the ≤11.2 bound — a faster int4 GEMV (O4's prerequisite) is now worth ~12.8 ms/token on this model's own decode | [56 §4](56-full-model-mixed-precision-study-plan.md) |
...
| 9 | ~~**R1** — the supertile first increment~~ **done 2026-08-22** — ported iron's partial-sum-staging pipeline; runs at the layer's width; device 26.02 ms vs `coarse`'s tail 4.66 ms, 5.6× (band-serial weight re-streaming); [31 §2.3](31-resident-tail-r1-record.md) | done |
| 10 | ~~**REEX** — the re-execution family matrix~~ **done 2026-08-22** — LOAD_PDI parity, fixed in `AIRRtToNpuPass` behind a failing lit; [57 §1.5](57-inference-path-optimizations-from-hexagon.md) | done |
| 11 | ~~**5b** — column-owns-heads GEMV kernel~~ **done 2026-08-23** — `mv_heads.cc` epilogue kernel, QKV stage 4 → 2 launches; 13.0 → 13.4–13.5 tok/s (−2.1…−3.5 ms/token vs −6 predicted: the whole-head tile serializes fill and drain, so the 8-row pipeline with in-core head accumulation won); verify PASS, reexec 7/7; [57 §5 item 5c](57-inference-path-optimizations-from-hexagon.md) | done |
| 12 | ~~**LLAMA** — Llama-1B mixed-partition head port~~ **done 2026-08-23** — the mixed partition never pays (15.10 → 15.15 ms at m_input 4; the "~0.36 ms" was byte arithmetic); `m_input 8` with uniform partitions landed instead: head 14.94 → 13.86–13.98 ms, −1.0 ms/token, verify PASS, reexec 7/7; [57 §5 item 5d](57-inference-path-optimizations-from-hexagon.md) | done |
| 13 | ~~**6b** — doc 56 H1a: model adapter + runner, schema v3, the kernel-scaling curve keyed by `Plan.sha`~~ **done 2026-08-23** (devq 568–576; one Codex round, devq 573, five blocking findings fixed) — `llms/shared/model_adapter.py`, `study/run_model.py` + `model_profiles.py`, schema v3 (host suite 656/656 in 30); walks 575 / 576 `complete: True`, `compare_roots OK`, verify PASS on the TIMED bytes at the timed prompt lengths; Qwen3-0.6B decode 12.0 / 10.6 / 7.3 tok/s at ctx 512 / 1024 / 2048, Llama-1B 10.8 / 10.0 / 8.9, prefill 1367–1371 and 1736–1739 tok/s at M 2048; the curve's M = 1024 point (1338–1359) runs the O+FFN cascade **forced fused-cast** (in its plan hash) and M = 512 hit a second wall (the registry's `tile_n 96` gate/up row vs one `mm.o` per ELF) — both walls named in [56 §4](56-full-model-mixed-precision-study-plan.md); a drain-capable cascade is the repair | done |
