...
`[2026-08-12]` **The open rows are 6c, 8, 9, 10, 12, 13 and 14, and they are not sequenced** — 6c
...
| **15** | **A THIRD unguarded latency gate — `compare_roots.py` — and it is the one trap 0's closing rule names** `[2026-08-12]`, found by sweeping for siblings while closing item 13. `study/compare_roots.py:138` gates on `avg_latency_ms` (with `effective_gflops_per_sec` and `avg_power_w`), fails at 15% drift for `hybrid`/`runlist`/`fused_elf` and 35% for `offload` (`:142-148`), and `main()` returns 1 (`:411`). **Verified: no `pmode`/`turbo`/`power_mode` match anywhere in the file.** A Turbo-vs-`Default` pair drifts ~1500–2000%, so a pmode change alone prints `VERDICT: PROBLEM` — and this is the tool that **compares two recorded roots**, i.e. exactly the splice trap 0 forbids ("never splice a comparison across a pmode change"). `compare_manifests()` (`:219`) diffs only the `git` and `toolchain` blocks. **It cannot be fixed the way 13 was**: `require_turbo()` reads the mode NOW, and this compares two runs recorded EARLIER — so it is **blocked on [34](34-phase-g-scoping.md) M4**, stamping the measurement condition into the manifest (verified: no pmode in `manifest.py` or `schema.py`). Do M4 first, then have `compare_manifests` refuse a pmode mismatch the way leg 4 now does. **Reported, not fixed** — item 13's brief was the two gates it named | Blocked on M4 (~1 h), then small | [34 §M1, §M4](34-phase-g-scoping.md) · trap 0 |
