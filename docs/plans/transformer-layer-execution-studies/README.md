...
| F — study harness | `execution-smoke-test` yields ≥1 `run_status=passed` row per measurement CSV | **in progress**, ~~on `exper/phase-f-study-harness` (a worktree, unmerged)~~ **`[2026-08-11]` correction: there is no unmerged worktree.** `exper/phase-f-study-harness` (tip `4775722e`) is a full ancestor of this branch, 0 unmerged commits — all Phase F work is here. **The gate itself passes on hardware over all four modes** as of 2026-08-08 — `smoke_gate` PASS, `manifest complete: True`, all four distinguishability clauses hold on the measured vectors ([09](09-phase-f-study-harness.md)). Work items 1, 2, ~~5~~, 6, ~~7~~, 8 done (5 without pytest, 7 scoped to result trees). **`[2026-08-11]` item 4 is four of five and item 3's PORTABLE half is done**: `study/resource_usage.py` (`core_to_core_flows` verified on real artifacts in both directions — 16/40 space-multiplexed on a fresh norm-tail compile, 0/116 time-multiplexed on `transformer_layer`; devq job 238), `component_groups.py`, `run_lock.py`, `cases.py`, `power.py` (root-free RAPL/hwmon backends, verified live), `compare_roots.py`, `select_rows.py`. Host suite 103/103 → **229/229 in 17 modules**. **What remains: `memcpy_bandwidth` (needs a multi-core AIR memcpy operator that does not exist) and the plot/analysis tier** — `regenerate_plots.py`, `roofline/{run,test}.py`, every `plot_*.py` — **still blocked**: matplotlib/pandas/seaborn are absent and must not be installed while gates run |
...
item 6b, then phase G and the two goals.** How each front landed: (1) **Item 3's remainder — the flip itself
...
arm stopped by **wall 4, shim BD exhaustion** — 24 refill tasks against tile (1,0)'s 16 BDs,
the J1 wall measured on R1, deterministic and loud. That is now **item 6b**, the resident
tail's one remaining blocker. (3) **Item 7 CLOSED
...
| **6b** | **Shim BD exhaustion on R1's down-feed refills — the J1 wall, now measured on R1** `[2026-08-11]`, **and measured to be independent of the fusion shape**: the gate reproduced the identical refusal (`'aiex.dma_configure_task' op Too many simultaneously active buffer descriptors on tile (1,0), which supports up to 16`, same `npu.air.mlir` line) under both the first fix's all-on-one-stream fusion and the revised per-side form. The invariant is the module's: 4 × 6 = 24 refill tasks, one `dma_configure_task` per iteration, no free/await reuse, however the channels are grouped. NOT a regression from 6a: the 24 transfers are semantically required (the consumer does 24 gets), and the pre-fix debris carried the same total without ever reaching this allocator. Candidate fix is [23 §4](23-rules-and-open-items.md)'s (loop-shaped shim BD programs or free/await insertion), unclaimed since J1 hit it at 6-vs-64 trips; R1 needs 24-vs-16, a much smaller gap. Doc 31's builder-side dodges do NOT apply — they change who fuses, not the task count | A compiler item; gates via 6's re-parked recipe | [31](31-fused-resident-tail.md) §status · [23 §4](23-rules-and-open-items.md) |
...
customer needing 24-vs-16). After it: re-arm `run_npu2_ffn_resident_peano.lit`, one gate run,
measured atol into the SPECS row. Beyond the resident tail: **phase G** (unattended runner +
CI) and the two goals, both untouched. Housekeeping for the operator: the `install-xrt`
