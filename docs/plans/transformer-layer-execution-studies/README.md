...
`[2026-08-12]` **The open rows are 6c, 12, 17, 20 and 21, and they are not sequenced** — 6c
...
| **20** | **`tee /dev/stderr` inside a devq job silently DESTROYS the job log — and the summary still says PASS** `[2026-08-12]`, found while gating q4_0 step 6. devq holds the log open `O_APPEND` on one fd; `/dev/stderr` re-opens the same regular file at offset 0, so the tee overwrites from the top. **Job 266 lost 10 of its 13 legs this way and still reported PASS** — the verdict survived because it is printed last, which is exactly what makes it dangerous. On a project whose rule is *no claim without an artifact*, a construct that shreds the artifact while preserving the verdict is the worst possible failure shape. Worked around in that job by capturing to a file (job 267 is the intact artifact), but nothing prevents the next author reaching for the same idiom | Small: refuse or document it in `devq.sh`'s header, beside its existing `run`-not-`submit` warning | devq |
