...
| H10 — non-constant BD offsets | `gate-h.sh` five legs, plus three objective clauses: an IV-dependent L2 offset refused by message, a constant one still compiling, an L3-side moving offset still compiling | **staged, not started** — spec at [24](24-phase-h10-non-constant-bd-offsets.md), `PL_PHASES_IN_SCOPE` already set, objective check verified FAILING on today's compiler |
