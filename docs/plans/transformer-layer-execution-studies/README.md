...
shipped models last verified 2026-08-10 — their re-run under the new install is owed (item 1(c)).
...
**Three fronts are open, none blocking the others.** (1) **Item 3, the `offload` default flip,
now decidable at the canonical shape**: the 4096 wall fell completely on 2026-08-11 — the
multi-launch dispatch experiment answered NO (**in-stream `load_pdi` faults the firmware**,
`fatal_error_type 0x10`; the scoped `--expand-load-pdis` fallback proved a no-op and was
falsified in doc 29), and route 3 landed instead: the shared chain pins a `fused-cast` winner to
the shape's `drain` row, runs **10/10 clean at 4096 with `context_loads 1` for 30 dispatches**,
and `run_npu2_offload_peano.lit` gates all three recipes at 4096. The flip forces the four-mode
re-walk — the first *un*-conditional cost-decomposed comparison, with Turbo a checked
precondition. (2) **Item 6a, the `air-fuse-channels` crash fix** (trap 4): the resident tail's
R1 is built and structurally green, and this pinned use-after-free is the only thing between it
and its device gate. (3) **Item 7, the `addnorm` two-pass phase.** Also owed, pure machine time:
item 1(c)'s ten-model re-run under the new install.
...
| **6** | **The `fused` resident tail — R1 BUILT 2026-08-11, device gate blocked on item 6a.** The one-segment FFN interior (up-proj + GeLU + J7b's down ring, three herds per 64-row band) is structurally green: hermetic probe 8/8 clauses, dataflow emulated element-exact in f64 (5.5e-12), 5/5 host tests with their own lit, both kernel objects compiling. All three named seams engineered (retile via the shim's 4-D pattern at zero cost, H in the down ring's K order, GeLU→down memtile fan-out by port arithmetic). **Its device gate is written and PARKED `UNSUPPORTED`** behind the `air-fuse-channels` crash (item 6a). After the unblock: delete the one `UNSUPPORTED` line in `run_npu2_ffn_resident_peano.lit`, run the gate via devq, replace the SPECS row's provisional atol with measured figures | Unblock (6a), then one gate run + the remaining increments | [31](31-fused-resident-tail.md) §status · [31a](31a-resident-byte-floor.md) |
| **6a** | **`air-fuse-channels` segfaults on ≥3 mutually-mergeable sibling channel nests** — an H9/H10-style compiler item, pinned to its minimal shape: N sibling same-bounds `scf.for` nests each carrying one auto channel; **N=2 fuses cleanly 5/5, N=3 crashes 5/5**. Mechanism consistent with a use-after-free in the NFL merge path (`nfl_erased_ops` queried after `wrapRegionsWithForLoops`; the O(N²) pairwise loop revisits erased ops), `mlir/lib/Transform/AIRDependencyScheduleOpt.cpp`. Under aircc the outcome is an **ASLR coin toss** (trap 4 below); deterministic via `air-opt --air-fuse-channels` on the pre-fuse dump, 10/10. Reproducer: `agents/probes/probe_fuse_channels_sibling_nests.py --nests 3 --keep out.mlir`. Alternative if the fix stalls: doc 31 records two unverified builder-side dodges (staggered per-nest bounds; restructure to ≤2 mergeable siblings) | A compiler fix with a shipped reproducer; gates via 6's parked recipe | [31](31-fused-resident-tail.md) §status |
...
fronts, none blocking the others: **item 3** (the `offload` default flip, now decidable at 4096,
plus its four-mode re-walk — the first unconditional cost-decomposed comparison); **item 6a**
(the `air-fuse-channels` crash fix — a compiler item with a shipped reproducer, and item 6's
only blocker); **item 7** (the `addnorm` two-pass phase). Item 1(c)'s ten-model re-run is pure
machine time — queue it when the device is quiet. ~~1b~~ resolved (pmode), ~~2, 4, 5~~ done
2026-08-10, item 1 done save (c), item 6's R1 landed 2026-08-11.
