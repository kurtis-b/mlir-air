...
`[2026-08-12]` **The open rows are 6c, 12 and 21, and they are not sequenced** — 6c
...
| **21** | **Wall 7 — R1 still hangs with every lock conserving** `[2026-08-12]`. With wall 6 fixed the numeric arm times out again: `ERT_CMD_STATE_TIMEOUT`, `txn_op_idx 0xFFFFFFFF`, `ctx_pc 0x28B060AD`, `fatal_error_type 0x0` — a clean hang, not a firmware fault. **What is known is mostly what it is NOT**: not an ordering defect (wall 5, closed by E1 — `w_down` folds to one monotone BD), not a BD-liveness defect (wall 4, closed by 6b), and **not a lock-count defect of this class** — the whole-module audit finds none left. **The negative control is INCONCLUSIVE, not passing**: it times out before it can compare, so the gate currently has no working discriminator at all, which is the first thing to fix. `ctx_pc` is a new datum no previous wall produced and is the obvious thread to pull. This is the **fourth** wall behind R1 and each has been real and distinct, so the honest prior is that the one-segment residency composition has more of them | A device-debug item; it needs a discriminator before it needs a fix | [37](37-wall-5-order-seam-design.md) · [31 §Wall 5](31-fused-resident-tail.md) |
