...
| # | Work | Size | Spec |
|---|---|---|---|
| **1** | **The xclbin multi-launch instruction streams — COMPILE HALF DONE 2026-08-10** (`623768f2`): the five-shape chain builds at 4096; a multi-launch module's per-device outputs are repackaged into the unchanged single-artifact contract (main orchestration stream with per-launch `load_pdi`, PDIs merged kernel-less into the partition; fixture `test/xrt/56`). **Remaining, in order**: (a) activate in `install-xrt` — the update was blocked by the session's permission classifier and needs the operator (`cp python/air/backend/xrt.py install-xrt/python/air/backend/xrt.py`); (b) the hardware experiment: no multi-launch xclbin has ever DISPATCHED in this tree — in-stream `load_pdi` against partition-resident PDIs is aiecc's default lowering, unvalidated; fallback (`--expand-load-pdis` + an aircc passthrough) scoped in doc 29; (c) re-run the offload suite + ten-model regression under the new install (the regression on the OLD install passed 2026-08-10: suite 30/30, models 10/10) | (a) is one command; (b) one gate run; (c) machine time | [29 §The 4096 wall](29-offload-n-streams.md) |
| **1b** | **The machine's `hw_context` load cost rose ~30× overnight** — ~78–80 ms/load measured from `runlist` and `offload` independently against ≤2.6 ms/load implied by 2026-08-09's records, bisected to the environment (pre-today code + warm caches reproduces it). Diagnose: reboot / `amdxdna` reload, then ONE offload rung at 1024 and compare against doc 29's 164–183 ms. Until then **no cross-day latency comparison is valid**, and [32](32-cost-decomposed-ladder.md)'s latency section is conditional | Small, but it gates every latency claim | [32](32-cost-decomposed-ladder.md) |
...
Items 1–3 are a chain and are all `offload`. ~~Items 4 and 5 are independent of everything and of
each other.~~ `[2026-08-10]` **Items 4 and 5 are done** — both landed with their gates re-run on
hardware the same day. **Nothing below is blocked on anything above.**
...
| `offload` | **Corrected and gated on BOTH halves** as of 2026-08-09. The LINEARITY half (2026-08-08): both attention matmuls on device, only softmax / both LayerNorms / GeLU on the host. The RECONFIGURATION half (`93e15a64`, [29](29-offload-n-streams.md)): five GEMM shapes in **one xclbin**, `context_loads 1` against the ELF path's 30, dispatch vector unchanged by design. `_evict_context`'s 30 reloads were the maximum reconfiguration cost in the mode defined to minimize it | **`[2026-08-09]` Gated** — the claim is enforced on both paths and verified failing. Two things left. The shared path **builds only where every module is single-launch**, which is 1024 and not the mode's own 4096 gate; the fix is a per-launch instruction-file name in `python/air/backend/xrt.py` and is its own phase. And **the default is still ELF**: switching packaging removes this mode's variance (eviction is the leading candidate but is not isolated — the switch also changes the ABI), the shared path trades ~20% of best-case latency at 512 for it, and flipping re-dates every recorded `offload` number |
...
reproduces it. See [32](32-cost-decomposed-ladder.md) and queue item 1b — fix the machine before
trusting any latency, and re-measure a whole comparison after, never splice.
