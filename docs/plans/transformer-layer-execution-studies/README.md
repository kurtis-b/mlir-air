...
| `offload` | **Half corrected and gated** 2026-08-08. The LINEARITY half is done: both attention matmuls on device, only softmax / both LayerNorms / GeLU on the host. **`[2026-08-09]` It currently pays the MAXIMUM reconfiguration cost, not the minimum** — `_evict_context` (`offload.py:465`) reloads the `hw_context` before every dispatch, 30 times per layer. That eviction is measured, but its **scope is narrower than its docstring claims**: the corruption is `elf`+`[2,2]` specifically, and `xclbin`+`[2,2]` reuses cleanly (`probe_context_reuse.py`). So the 30 reloads are removable, and they are the leading candidate for this mode's 120% intra-walk latency drift ([27](27-common-ladder-result.md)) | **N instruction streams under one xclbin** — untouched, and it is where the mode's reconfiguration-minimizing claim actually gets tested. Already plumbed (`xclbin_input`, `xrt.py:80`, `:372-374`), never dispatched. Touches `llms/shared/infra/dispatch.py:685`, shared with `fused`. **`[2026-08-09]` BLOCKED, and one level lower than the plan assumed.** Two questions were asked and they answered opposite ways. Context reuse is **fine**: under the xclbin ABI, at the mode's existing `[2,2]`, a kernel re-executes bit-identically (`probe_context_reuse.py`). But **`--xclbin-input` chaining does not work**: two GEMMs packaged into one xclbin both appear in `get_kernels()`, and only the one compiled **FIRST** executes correctly. Reversing the compile order reverses which one breaks, so it is positional, not shape-specific — and the second kernel's failure mode depends on its shape, `ERT_CMD_STATE_TIMEOUT` for one and **garbage at `mean_rel_L1` 1.41 with no error raised** for the other. Each compiled standalone runs correctly, so the only variable is the chaining (`probe_one_xclbin_n_streams.py`). **So this is not a dispatch-layer phase.** The `plan_submissions` split rule (key on array-configuration identity, `artifact_of` as the default proxy) and `ensure_loaded` context sharing remain correct designs with nothing to run on. The blocker is `aircc --xclbin-input` and below |
...
| Can N instruction streams share one xclbin? | **No, not on today's `--xclbin-input`.** Two GEMMs chained into one xclbin both appear in `get_kernels()`, but only the one compiled FIRST executes correctly; reversing the compile order reverses which breaks. The second either times out or returns garbage at `mean_rel_L1` 1.41 **with no error raised**, depending on its shape. Each is correct compiled standalone, so the chaining is the only variable. Doc 26 records this mechanism as "already plumbed but never dispatched" — it is plumbed and it does not work, which moves the phase from a dispatch-layer change to a compiler/packaging one. | `agents/probes/probe_one_xclbin_n_streams.py` |
