...
| J3 | ~~**Walk the `baseline_768` sequence ladder** for all four modes.~~ **`[2026-08-08]` DONE — and it produced a crossover.** Two independent 16-rung walks, 512/1024/2048/4096 × four modes, 16/16 passing each time: **`fused` leads at 512 and 2048, `coarse` leads at 4096**, and the log-log slopes split on **attention placement** (device 1.03–1.17, host 1.23–1.27) rather than on dispatch structure. At 1024 the two walks disagree and the pair is indistinguishable — a first-draft claim this document would otherwise have carried. Full result, spread and limits in [25](25-first-study-result-sequence-ladder.md). | A tradeoff analysis at a single shape has no curves and therefore no crossover — which is the result the study exists to produce. | — |
...
| J6 | **Phase F cost metrics.** Latency, power, resource usage, GFLOPs — the seven studies in [09](09-phase-f-study-harness.md). **`[2026-08-08]` Latency is measured** across four modes × four lengths; **power needs a decision before code.** Iron's backend calls `sudo -n turbostat` and passwordless sudo is unavailable here, `turbostat --no-msr` emits no samples, and no sensor on this platform measures the NPU — RAPL `package-0` is CPU package and `amdgpu`'s `PPT` is the SoC rail, both root-free. So a cross-mode power comparison largely measures *host* work, which is the very thing the modes differ in. See [09](09-phase-f-study-harness.md). | The dispatch vector is a proxy for cost, not cost. Until this lands there are no tradeoffs, only structure. | J2, J3 |
