...
| E `llms/` + kernels | 14,477 | 7,233 | 6,236 | 1,583 | 20,127 | int4_gs plumbing (this PR, 40; int4 Q4_0 plan PR 1); Qwen3-0.6B decode QKV at 2 launches (#19, 257); 2-launch QKV ELF builder (#17, 479); qkv-heads kernel + layout (#16, 395); rms/qkv host-ABI seam refactor (#15, main-side structure-only, net +3 — not branch lines); verify_runner host tests (#14, 412); next: shared decode_qkv4 + test, then the qwen3 / llama32_1b_int4 / smollm2 int4 model rows, int4_awq q4_0, registry rows (data, ≈11 PRs) |
| D `transformer_layer/` | 0 | 0 | 69,330 | — | 0 | excluded (Q1) |
| C plan docs | 0 | 0 | 12,805 | — | 0 | excluded (Q3) |
| other | 0 | 29 | 41 | 0 | 29 | rides with B |
| **total** | **17,436** | **13,361** | **94,437** | **4,703** | **26,094** | 35 PRs include-only; 62 at full refactor size |
