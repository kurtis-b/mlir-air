...
`air.shrinkage` attribute). The file is absent from **both** main and the tag at that path, so
there is nothing to port and nothing to verify — recorded as moot rather than as a closure.
