...
the systematic-debugging fallback: capture a minimal repro, bisect the last relevant change, isolate the failing layer or kernel, and document the blocker in the model TODO instead.
...
`programming_examples/llms/shared/infra/stitching.py` already prefixes every SSA
...
`programming_examples/llms/shared/infra/stitching.py`. The fused builders in
`programming_examples/llms/shared/builders/` apply this wrapper around every bare
