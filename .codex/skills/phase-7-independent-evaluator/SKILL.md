...
1. **Using a fresh Codex subagent when multi-agent tools are available**
   with no context from the deployment; otherwise recording that the audit
   was not fresh-context independent
...
1. **`make verify` audited** (anti-reward-hacking): independent auditor has read the
...
2. **`make verify` PASSES under the fresh-context audit run**: token-set gate
...
If the independent auditor declares PASS without showing measured numbers OR without
auditing the verify code path, the report is REJECTED — run a fresh audit with
...
- `<model>/docs/development_progress/` + the `kernel_registry` rows with
  Used by = `<model>`
  — Phase 1 catalog (independent auditor compares its measurements against this)
- `programming_examples/kernel_registry/supported_kernels.md`
  — kernel-by-kernel ground truth

## Workflow

### Step 1: Prepare a fresh-context audit with independence + audit constraints

Use a fresh Codex subagent when multi-agent tools are available. If no
multi-agent tool is available, continue in the current context only after
recording in the report that the audit was not fresh-context independent.
Critical instructions in the audit prompt:
...
| Independent auditor reads LESSONS/progress before measuring | Audit prompt was not strict enough | Reject the report; run a fresh audit with stricter instructions |
