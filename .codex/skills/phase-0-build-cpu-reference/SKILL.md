...
2. **`<model>_cpu_helpers.py`** — the small set of NumPy helpers that
   *production* prefill/decode import (default: `rms_norm`,
   ...
   `programming_examples/llms/shared/infra/<kernel>/run.py` harness (Phase 1 uses those).
...
1. **Per-kernel references (Phase 1)**: HF cannot expose a single kernel's
   ...
   harness (`programming_examples/llms/shared/infra/<kernel>/run.py`) carries its own NumPy F32
...
do NOT go here — they live in each `programming_examples/llms/shared/infra/<kernel>/run.py`. If a
