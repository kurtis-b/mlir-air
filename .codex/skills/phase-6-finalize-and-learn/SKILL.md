...
3. Did this deployment surface a **new skill-chain change**? Edit the
   relevant `.codex/skills/<phase>/SKILL.md` directly (git history is
...
- `<model>/ARCHITECTURE.md`: write or update with final summary (model
  ...
  `ARCHITECTURE.md` — the architecture file is tracked, so it ships in the PR.
- **Hand off to Phase 7**: the deploy-new-llm orchestrator now runs
