---
name: deploy-new-llm
description: "Entry point for deploying a new decoder-only LLM on AMD NPU2. Invoked by the user as `$deploy-new-llm HF model ID [--name dirname] [--target npu2|npu1] [--dtype bf16|fp16]`. Bootstraps the per-model workspace, validates architecture is in scope, and dispatches the 7 per-phase skills with the gate of each phase enforced by that phase's skill."
---
...
shared infra toolkit and gates on the shared `verify/`
...
shared infra toolkit (KernelCache, stitching,
...
├── ARCHITECTURE.md                  # model-specific guide (not agent-local notes; it ships in git)
...
| 7 | `phase-7-independent-evaluator` | Fresh-context Codex audit: use a fresh subagent when multi-agent tools are available, audit `make verify` (anti-reward-hacking) + re-run as primary gate; produce structured `evaluation_report.md` |
...
After Phase 6 PASSES but BEFORE the final hand-off, run the
`phase-7-independent-evaluator` skill as Phase 7. It re-derives every
correctness claim with a fresh Codex subagent when multi-agent tools are
available; otherwise it records that the audit was not fresh-context
independent. It produces `<model>/docs/evaluation_report.md`.
...
> "Running Phase 7 — independent evaluation. The evaluator will use a
> fresh Codex subagent when available, audit `make verify`
> (anti-reward-hacking), re-run it as the primary gate, and write a
> structured report. Expected runtime: 15-30 min."

Then invoke `$phase-7-independent-evaluator <model_dir>`.
