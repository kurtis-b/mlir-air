...
// Whole-head-per-iteration form (the core sees a head as ONE launch iteration
// of HEAD_DIM/m chunk calls, row_offset = chunk*m): the row_offset-0 call
// also copies B's tail [k, k+3*HEAD_DIM) into `scratch` while B is lock-held,
// and `qknorm_rope_head_bf16` runs after the chunk loop on the scratch.
void qkv_heads_chunk_scratch_bf16(uint32_t m, uint32_t k, uint32_t row_offset,
                                  const bfloat16 *__restrict a_in,
                                  const bfloat16 *__restrict b_in,
                                  bfloat16 *__restrict c_out,
                                  bfloat16 *__restrict scratch) {
  if (row_offset == 0)
    copy_head<bfloat16, 16, 3 * HEAD_DIM>(b_in + k, scratch);
  matvec_heads_vectorized<64>(m, k, k, a_in, b_in, c_out + row_offset);
}

// scratch = [lut | q_norm | k_norm]; kind 0 Q, 1 K, 2 V (out = c).
void qknorm_rope_head_bf16(bfloat16 *__restrict c, bfloat16 *__restrict scratch,
                           bfloat16 *__restrict out, float eps, int32_t kind) {
  if (kind < 2)
    qknorm_rope_head<bfloat16, 16, HEAD_DIM>(
        c, scratch + HEAD_DIM * (1 + kind), scratch, out, eps);
  else
