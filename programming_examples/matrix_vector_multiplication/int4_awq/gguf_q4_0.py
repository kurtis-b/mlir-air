...
# `blk.{0,1,10}.ffn_down.weight` -- against 165 pure q4_0 linears.
#
...
# `uint8_t *a_z` (`mv_int4_bf16.cc:88`) -- an INTEGER. So q4_1 is not
...
#
# Measured on `bartowski/SmolLM2-1.7B-Instruct-Q4_0.gguf` against the bf16
# `HuggingFaceTB/SmolLM2-1.7B-Instruct` weights, rms error over rms weight:
#
#     the 165 pure-q4_0 ffn_down tensors (the error llama.cpp ACCEPTED)
#                                              0.0828 .. 0.0853
...
def q4_0_payload_for(g, name, reference=None):
    ...
    reference. Route (c) above is a measured 32% outside the accepted error
    family, so falling back to it silently would degrade three layers of the
    model in a way nothing downstream could see -- the packer would succeed and
    the tokens would just be slightly worse.
    ...
    if reference is None:
        raise ValueError(
            ...
            f"{kind}'s fractional zero-point is not representable; transcoding "
            "from it measures 0.111 rms/rms against an accepted band of "
            "0.083-0.085. Pass the original bf16 weights for this tensor."
        )
...
def promoted_tensors(g):
    ...
    What a packer needs reference weights for. Named separately from
    ``checkpoint_traffic`` because that function answers "what does this cost
    to move" and this one answers "what can this kernel not consume".
    """
    out = {}
...
def self_test(K=256, M=64, seed=42, verbose=True):
    ...
    p_asym = pack_inputs(A_q, A_s, A_z, M, K, QK4_0, M_TILE, K_CHUNK, N_CORES, M)
    p_sym = pack_inputs(
    ...
    if (
        abs(t_a["bits_per_weight"] - 4.75) > 1e-9
        or abs(t_s["bits_per_weight"] - 4.50) > 1e-9
    ):
        raise AssertionError(
    ...
    #     and both routes are then byte-identical by construction (measured:
    #     rms equal to 12 significant figures). The transcode penalty exists
    #     only because q4_1 is ALREADY an approximation of the float source, so
    #     the source here is unconstrained noise and is quantised to q4_1 by
    #     llama.cpp's own rule (`d = (max-min)/15`, `m = min`).
...
def _main():
    ...
    if args.traffic:
        ...
        print(
            "  pure-q4_0 linears: %d tensors, %d weights"
            % (tr["n_tensors_q4_0"], tr["n_weights_q4_0"])
        )
        print(
            "  asymmetric (Z present): %d B = %.2f MiB  (%.3f bits/weight)"
            % (
                tr["asym_bytes"],
                tr["asym_bytes"] / MiB,
        ...
        print(
            "  symmetric  (no Z)     : %d B = %.2f MiB  (%.3f bits/weight)"
            % (
                tr["sym_bytes"],
                tr["sym_bytes"] / MiB,
        ...
        print(
            "  Z plane                : %d B = %.2f MiB  (%.3f bits/weight)"
            % (
                tr["z_bytes"],
                tr["z_bytes"] / MiB,
        ...
        if None not in (nl, emb, ff, nh, nkv):
            ...
            print(
                "  asymmetric %d B = %.2f MiB (%.3f b/w) | symmetric %d B = "
