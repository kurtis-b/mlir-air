...
//   - matvec_int4_bf16_packed_store(packed, b, c):                overwriting
//         variant (c = ...), saves a zero call when K = K_CHUNK.
//   - matvec_int4_bf16_packed_b_offset(packed, b, b_offset, c):  accumulating
//         variant with a base offset into b (lets callers tile k against a
//         single L1 buf).
...
// Inner vector width `r` of the four GEMV bodies. At r = 64 the weight load
...
// `results/item23-int-gemv-20260826/isa_probe/probe_r64.dis`). The arithmetic
// is unchanged -- only the lane count of the per-group accumulate differs, so
// there is no format, layout, packer or accuracy-class consequence.
//
...
// SmolLM2-1.7B int4 decode (q4_0 blocks, `smollm2_1_7b_int4_decode.py:57`) and
...
static_assert(DIM_GS % DIM_R == 0,
              "DIM_R must divide DIM_GS (r=64 needs a group size multiple of 64)");
...
// Overwriting variant: stores instead of accumulating. Saves a separate
// zero call when the caller doesn't K-chunk (single-chunk matvec).
template <unsigned m, unsigned k, unsigned gs, unsigned r>
void matvec_int4_bf16_store_impl(uint8_t *__restrict a_q,
                                 bfloat16 *__restrict a_s,
                                 uint8_t *__restrict a_z,
...
#pragma clang loop unroll(full)
    ...
    c[row] = (bfloat16)aie::reduce_add(acc.template to_vector<float>());
  }
}
...
// keep one big input buffer and tile over k via subview-without-cast.
void matvec_int4_bf16_packed_b_offset(uint8_t *packed, bfloat16 *b,
                                      int b_offset, bfloat16 *c) {
  constexpr unsigned Q_BYTES = DIM_M * (DIM_K / 2);
  constexpr unsigned S_BYTES = (DIM_K / DIM_GS) * DIM_M * 2;
  uint8_t *a_q = packed;
  bfloat16 *a_s = reinterpret_cast<bfloat16 *>(packed + Q_BYTES);
  uint8_t *a_z = packed + Q_BYTES + S_BYTES;
  matvec_int4_bf16_impl<DIM_M, DIM_K, DIM_GS, DIM_R>(a_q, a_s, a_z, b + b_offset,
                                                  c);
}

// Overwriting matvec entry for callers that don't K-chunk. Folds the per-tile
// zero into the kernel epilogue (c[row] = s, not c[row] += s).
void matvec_int4_bf16_packed_store(uint8_t *packed, bfloat16 *b, bfloat16 *c) {
  constexpr unsigned Q_BYTES = DIM_M * (DIM_K / 2);
  constexpr unsigned S_BYTES = (DIM_K / DIM_GS) * DIM_M * 2;
  uint8_t *a_q = packed;
  bfloat16 *a_s = reinterpret_cast<bfloat16 *>(packed + Q_BYTES);
...
void matvec_int4_bf16_packed_sym(uint8_t *packed, bfloat16 *b, bfloat16 *c) {
  ...
  uint8_t *a_q = packed;
  bfloat16 *a_s = reinterpret_cast<bfloat16 *>(packed + Q_BYTES);
  matvec_int4_bf16_sym_impl<DIM_M, DIM_K, DIM_GS, DIM_R, (int8_t)Q4_0_ZP>(a_q, a_s,
                                                                       b, c);
}
