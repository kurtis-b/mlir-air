//===- mv_int4_bf16_r64.cc - the SHIPPED bf16 GEMV body at r=64 -----------===//
//
...
// width r = 64 instead of the shipped 32. The arithmetic is character-for-
// character the shipped kernel's; only the vector width differs.
//
...
// 32-byte source. Measured on one gs=128 group (isa_probe/probe_r64.dis):
//
//     r = 32 (SHIPPED) : 47 vector ops, 0 vldb.unpack, 4 separate vunpack
//     r = 64           : 34 vector ops, 2 vldb.unpack, 0 separate vunpack
//
...
static_assert(DIM_GS % 64 == 0, "this control requires gs to be a multiple of 64");

template <unsigned m, unsigned k, unsigned gs, unsigned r>
static void matvec_int4_bf16_r_impl(uint8_t *__restrict a_q,
