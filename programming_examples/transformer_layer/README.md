...
| `builders/test_ffn_resident.py` | Host-only: **builds the module and INTERPRETS it** — every `air.dma_memcpy_nd`/`air.channel.put`/`get` at the offsets, sizes and strides the op carries, every `scf.for` at its real bounds, the three herds as four concurrent actors each, f64 throughout; **max \|y − ref\| 5.457e-12**. Two negative controls run on every invocation and are FileCheck-matched — the c-major w_down refill route E1 deleted (reproduces the real pre-E1 builder to the digit) and the retile's inner-stride swap. `[2026-08-12]` queue item 17: **until then it imported one packer and never built the module**, so it could not see the refill nest change at all. Models addressing and delivery ORDER only — not the lock or BD walls (items 6c, 18). The arm that stays live while the device gate is parked. No NPU, no aircc |
