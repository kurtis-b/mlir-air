...
### Transformer-layer execution study sweeps

The four sweep series (`baseline_768`, `baseline_512`, `baseline_1024`, `qwen3_0_6b` —
124 rows) are **not repeated here**. Every row lives in
[`details/GEMM_bf16_in_bf16_out.md`](details/GEMM_bf16_in_bf16_out.md), and the machine
source of truth is its companion `details/GEMM_bf16_in_bf16_out.json`, which
`registry_lookup.gemm_config(...)` reads. They were carried in three places at once;
this file keeps the index, the detail page keeps the data.
