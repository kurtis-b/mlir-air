...
from gemma3.core.reference import apply_rope_halfsplit, geglu, qk_norm, rms_norm
...
def nonlinear_registry(config: Gemma3TextConfig | None = None) -> tuple[Gemma3NonlinearSpec, ...]:
    ...
    return (
        ...
        Gemma3NonlinearSpec(
            operation="rope",
            model_status="standalone-npu-reuse-candidate",
            implementation_path="programming_examples/gemma3/gemma3/kernels/rope_halfsplit.py",
            compile_lit="programming_examples/gemma3/tests/lit/kernels/run_rope_halfsplit_compile_only.lit",
            ...
            compile_status="compile-lit-available",
            hardware_status="hardware-smoke-pass-rows4-head256-elf",
            tolerance="rtol=5e-2 atol=5e-2 hardware-smoke",
            timed_window_status="unmeasured-host-fallback",
            fallback="host CPU reference until Gemma model wiring uses the validated standalone kernel",
            notes="Standalone half-split RoPE ELF compile-and-run passed for rows=4 head_dim=256.",
        ),
        Gemma3NonlinearSpec(
            operation="mlp_activation",
            model_status="standalone-npu-reuse-candidate",
            implementation_path="programming_examples/gemma3/gemma3/kernels/geglu.py",
            compile_lit="programming_examples/gemma3/tests/lit/kernels/run_geglu_compile_only.lit",
            cpu_reference="gemma3_reference.geglu",
            tensor_contract=f"GeGLU gate/up vectors of hidden_dim={config.hidden_dim}",
            compile_status="compile-lit-available",
            hardware_status="hardware-smoke-pass-n6912-tile288-elf",
            tolerance="rtol=1e-1 atol=5e-2 hardware-smoke",
            timed_window_status="unmeasured-host-fallback",
            fallback="host CPU reference remains in blocked paper-cell paths until full model-loop timing uses this staged kernel",
            notes="Standalone ELF compile-and-run passed for the 1B MLP activation shape n=6912 tile_n=288 with Peano/XRT; layer-0 staged full-layer launch also passes.",
        ),
        Gemma3NonlinearSpec(
            operation="residual_add",
            model_status="standalone-npu-reuse-candidate",
            implementation_path="programming_examples/gemma3/gemma3/kernels/residual_add.py",
            compile_lit="programming_examples/gemma3/tests/lit/kernels/run_residual_add_compile_only.lit",
            cpu_reference="gemma3_reference residual add sites",
            tensor_contract=f"elementwise add over emb_dim={config.emb_dim}",
            compile_status="compile-lit-available",
            hardware_status="hardware-smoke-pass-n1152-tile288-elf",
            tolerance="rtol=5e-2 atol=5e-2 hardware-smoke",
            timed_window_status="unmeasured-host-fallback",
            fallback="host CPU reference until Gemma model wiring uses the validated standalone kernel",
            notes="Standalone residual_add ELF compile-and-run passed for n=1152 tile_n=288.",
        ),
    )
