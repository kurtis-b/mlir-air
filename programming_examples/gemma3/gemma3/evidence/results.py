...
from gemma3.evidence.nonlinears import measure_cpu_contracts, paper_match_blockers
from gemma3.evidence.paper_compare import load_targets, target_by_id
from gemma3.evidence.power import capture_power_snapshot
...
from gemma3.evidence.real_execution import Gemma3ExecutionError, run_torch_benchmark
...
def model_runner_record_from_plan(plan: Any) -> dict[str, Any]:
    return {
        ...
        "kernel_argument_binding_status": getattr(plan, "kernel_argument_binding_status", None),
        "kernel_argument_binding_count": getattr(plan, "kernel_argument_binding_count", None),
...
def load_staged_npu_diagnostic(model_variant: str) -> dict[str, Any] | None:
    if model_variant != "gemma3-1b":
        return None
    try:
        from gemma3.probes.full_layer import DEFAULT_FULL_LAYER_PROBE_EVIDENCE, has_decode_full_layer_evidence
...
def load_decode_loop_diagnostic(model_variant: str) -> dict[str, Any] | None:
    if model_variant != "gemma3-1b":
        return None
    try:
        from gemma3.probes.decode_loop import DEFAULT_LOOP_PROBE_EVIDENCE
...
TILED_ATTENTION_STATS_EVIDENCE = (
    RESULTS_DIR / "gemma3_flowqkv_tiled_stats_1k_smoke.json"
)
...
def build_model_runner_record(
    *,
    ...
    if backend != "npu" or not artifacts_ready:
        return None
    try:
        from gemma3.npu.model_runner import build_model_runner_plan
        ...
        return {
            ...
            "kernel_argument_binding_status": None,
            "kernel_argument_binding_count": None,
...
def _self_test() -> None:
    result = build_paper_result(
        ...
        command=["python", "-m", "gemma3.evidence.results", "--self-test"],
    )
