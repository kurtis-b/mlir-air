...
from gemma3.npu.stitching import StitchSpec, stitch_module_text
from gemma3.paths import EXAMPLE_ROOT, PROGRAMMING_EXAMPLES_DIR, REPO_ROOT, RESULTS_DIR
...
DEFAULT_FUSED_DQP_OBJECT = EXAMPLE_ROOT / "build_peano" / "fused_dqp.o"
DEFAULT_ROPE_OBJECT = EXAMPLE_ROOT / "build_peano" / "rope_halfsplit.o"
DEFAULT_FLOWQKV_OBJECT = EXAMPLE_ROOT / "build_peano" / "flowqkv_single_token_q4_kv1_d256.o"
DEFAULT_THRESHOLD = 0.99
DEFAULT_LAYER = 0
DEFAULT_INGRESS_RESULT_JSON = RESULTS_DIR / "gemma3_1b_stitched_decode_ingress_probe.json"
DEFAULT_ATTENTION_O_RESULT_JSON = RESULTS_DIR / "gemma3_1b_stitched_attention_o_probe.json"
DEFAULT_POST_ATTENTION_RESIDUAL_RESULT_JSON = RESULTS_DIR / "gemma3_1b_stitched_post_attention_residual_probe.json"
DEFAULT_FFN_GATE_UP_RESULT_JSON = RESULTS_DIR / "gemma3_1b_stitched_ffn_gate_up_probe.json"
DEFAULT_GEGLU_DOWN_RESULT_JSON = RESULTS_DIR / "gemma3_1b_stitched_geglu_down_probe.json"
...
def _activate_builder_paths() -> None:
    weighted = str(PROGRAMMING_EXAMPLES_DIR / "weighted_rms_norm")
    example_root = str(EXAMPLE_ROOT)
    for path in (weighted, example_root):
...
def build_ingress_text(
    *,
    ...
    _activate_builder_paths()
    from gemma3.kernels.projection_qk_norm import build_module as build_qk_norm
    from ml_dtypes import bfloat16
    from gemma3.kernels.rope_halfsplit import build_module as build_rope
    from weighted_rms_norm import build_module as build_weighted_rms
...
def build_attention_o_text(
    *,
    ...
    return stitch_module_text(
        ...
        specs=(
            StitchSpec(flow_ir, "att", {0: 0, 1: 1}),
            StitchSpec(o_ir, "o", {0: 3, 1: 2, 2: 4}),
        ),
    )
...
def build_geglu_down_text(
    *,
    ...
    _activate_builder_paths()
    from gemma3.kernels.fused_dqp import build_paper_module
    from gemma3.kernels.geglu import build_module as build_geglu
    from ml_dtypes import bfloat16
...
def build_post_attention_residual_text(
    *,
    ...
    _activate_builder_paths()
    from ml_dtypes import bfloat16
    from gemma3.kernels.residual_add import build_module as build_residual_add
    from weighted_rms_norm import build_module as build_weighted_rms
...
def _pack_projection_for_herd_cols(weight, *, herd_cols: int, row_block_count: int | None = None):
    ...
    from gemma3.probes.full_layer import _repack_matrix_for_fused_dqp
...
def _run_ingress_hardware(args: argparse.Namespace) -> StitchedIngressProbeResult:
    ...
    from gemma3.core.artifacts import default_weights_dir
    from gemma3.probes.full_layer import _projection_tensor_keys
    from ml_dtypes import bfloat16
    import numpy as np
...
def _run_attention_o_hardware(args: argparse.Namespace) -> StitchedAttentionOProbeResult:
    ...
    from gemma3.core.artifacts import default_weights_dir
    from gemma3.probes.full_layer import _compile_flowqkv_single_token_kernel, _projection_tensor_keys
    from ml_dtypes import bfloat16
    import numpy as np
...
def _run_geglu_down_hardware(args: argparse.Namespace) -> StitchedGeGLUDownProbeResult:
    ...
    from gemma3.core.artifacts import default_weights_dir
    from gemma3.probes.full_layer import _projection_tensor_keys
    from gemma3.kernels.geglu import geglu_reference
    from ml_dtypes import bfloat16
    import numpy as np
