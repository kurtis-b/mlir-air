...
from gemma3.core.common import SUPPORTED_HERD_SHAPES, parse_herd_shape
from gemma3.paths import PROGRAMMING_EXAMPLES_DIR

BF16_MM_RUN = PROGRAMMING_EXAMPLES_DIR / "matrix_multiplication" / "bf16" / "run.py"
_BF16_MM_SPEC = importlib.util.spec_from_file_location("gemma3_bf16_mm_run", BF16_MM_RUN)
if _BF16_MM_SPEC is None or _BF16_MM_SPEC.loader is None:
    raise ImportError(f"could not load bf16 MM builder from {BF16_MM_RUN}")
_BF16_MM_MODULE = importlib.util.module_from_spec(_BF16_MM_SPEC)
_BF16_MM_SPEC.loader.exec_module(_BF16_MM_MODULE)
build_module = _BF16_MM_MODULE.build_module
