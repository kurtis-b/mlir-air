...
blocked by full 1B loop wiring, full-token runlisted decode, and fresh
paper-shape hardware reruns. The prior
...
production prefill handoff, full-token runlist, or NPU argmax gates. Staged
decode layer-0 correctness and
...
The existing stitched, single-token, tiled-stat, host-logits, and staged-loop
results in this directory are historical diagnostics. They should remain
described as non-paper-cell evidence unless their source JSON is regenerated by
...
The planned paper-cell decode result is valid only when all of these gates are
blocker-free in the same result set:

- Production NPU prefill K/V supplies decode; no HF, synthetic, probe, or
  host-replaced K/V is used.
- Decode uses `decode_execution_mode=runlist-full-token`.
- `runlist_entry_count=27`, with 26 full-layer entries plus one NPU
  final-norm/tied-embedding argmax entry.
- `runlist_execute_count == decode_tokens` and
...
- `intermediate_readback_count=0` and `dynamic_static_write_count=0`.
- FlowKV/FlowKV-SWA attention uses Q chunk size 1, KV chunks over the
  1024-token context, and `attention_reduction_mode=npu`.
- `logits_sampling_mode=npu` and `sampling_policy=argmax`.
- Correctness, timing, and aligned timed-window power metadata all pass.
...
- `gemma3_1b_decode_loop_probe.json`: compact Strix/XRT evidence for one staged
  ...
  cache and performs the merge on the host.
...
- `gemma3_flowqkv_tiled_stats_1k_smoke.json`: compact Strix/XRT evidence for a
  ...
  is host-side. The diagnostic decode-loop probe can call this path through
  `--attention-mode tiled-stats-1k`.
  The paper-cell production path uses FlowKV or FlowKV-SWA with NPU-owned
  softmax state/reduction over the production prefill K/V cache. Full
