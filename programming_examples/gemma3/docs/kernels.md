...
The canonical `programming_examples/gemma3` example contains MLIR-AIR/NPU2 kernel
wrappers for the patterns described in "Mapping Gemma3 onto an Edge Dataflow
Architecture". The goal is to keep each kernel concrete, testable, and easy to
iterate on while it is integrated into the Gemma3 model runtime.
...
From `programming_examples/gemma3`, with the usual MLIR-AIR NPU environment active:
...
make run-flowqkv
make run-flowkv
PYTHONPATH=. python3 -m gemma3.kernels.residual_add --compile-mode compile-only --output-format elf
PYTHONPATH=. python3 -m gemma3.kernels.rope_halfsplit --compile-mode compile-only --output-format elf
