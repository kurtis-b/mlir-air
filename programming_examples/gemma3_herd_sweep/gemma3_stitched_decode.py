...
def build_decode_ingress_plan(model_variant: str = DEFAULT_MODEL) -> DecodeStitchPlan:
    stages = (
        ...
        DecodeStitchStage(
            ...
            "planned-bridge",
            "q:32x32 k:8x32",
            "q_heads:4x256 k_heads:1x256",
            "Contiguous memref view bridge before weighted RMSNorm Q/K norm.",
        ),
        DecodeStitchStage(
            "qk_norm_rope",
            "planned-stitch",
            "q_heads:4x256 k_heads:1x256 + q/k norm weights + RoPE LUTs",
            "q_rope:4x256 k_rope:1x256",
            "Can reuse weighted_rms_norm and rope_halfsplit once view bridge exists.",
        ),
    )
