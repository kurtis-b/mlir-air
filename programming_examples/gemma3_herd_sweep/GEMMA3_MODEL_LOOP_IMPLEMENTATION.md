...
- Current Gemma3 work: `programming_examples/gemma3_herd_sweep`.
- Standalone Gemma3 kernel sketches: `programming_examples/gemma3_dataflow_kernels`.
- NPU model-runtime reference: `programming_examples/llama32_1b`.
- Paper target ledger: `programming_examples/gemma3_herd_sweep/paper_targets.json`.
...
| --- | --- | --- |
| Model/layer metadata | `gemma3_config.py`, `gemma3_artifacts.py` | Defines model variants, local/global attention metadata, dimensions, and artifact discovery. |
| Synthetic weights | `gemma3_weights.py` | Provides deterministic smoke inputs and Q4NX-compatible metadata. |
| CPU math references | `gemma3_reference.py` | Defines correctness semantics for projections, attention, nonlinear ops, residuals, logits, and KV updates. |
| Runtime manifests | `gemma3_runtime.py` | Records artifact names, static inputs, intermediate outputs, and fallback metadata without importing AIR/XRT. |
| Result contracts | `gemma3_results.py`, `gemma3_paper_compare.py`, `paper_targets.json` | Produces machine-readable local results and paper deltas. |
| Environment/power metadata | `gemma3_environment.py`, `gemma3_power.py` | Captures run conditions and timed-window power metadata. |
| Blocker ledger | `gemma3_reproduction_blockers.py` | Reports why a backend/model cell is not paper-comparable. |
...
- `gemma3_reference.py` implements synthetic CPU references.
- `gemma3_model_loop.py`, `gemma3_prefill.py`, and `gemma3_decode.py` run the
  synthetic model loop and record per-stage metadata.
- `gemma3_real_execution.py` runs real Gemma3 artifacts through HF/Torch on CPU.
- `gemma3_results.py` and `gemma3_paper_compare.py` turn those runs into local
  paper-cell records.
...
- `gemma3_real_execution.py` selects ROCm through Torch CUDA APIs when
  `backend=igpu`.
- `gemma3_power.py` records ROCm SMI graphics-package power when available.
...
| --- | --- | --- |
| Q4NX dequantization | `programming_examples/gemma3_herd_sweep/q4nx.py`, `programming_examples/gemma3_herd_sweep/q4nx_opt.cc` | Standalone validated/sweepable route. |
| BF16 tiled MM | `programming_examples/gemma3_herd_sweep/bf16_tiled_mm.py` | Uses the optimized AIE2P BF16 GEMM path. |
| FusedDQP | `programming_examples/gemma3_herd_sweep/fused_dqp.py`, `programming_examples/gemma3_herd_sweep/fused_dqp_opt.cc` | Decode projection candidate; paper route passes standalone, pipeline route is diagnostic-only. |
| FlowQKV | `programming_examples/gemma3_herd_sweep/flowqkv.py`, `programming_examples/gemma3_herd_sweep/flow_attention_opt.cc` | Prefill attention candidate. |
| FlowKV | `programming_examples/gemma3_herd_sweep/flowkv.py`, `programming_examples/gemma3_herd_sweep/flow_attention_opt.cc` | Decode attention candidate and `Q_CHUNK=1` specialization. |
| RoPE | `programming_examples/gemma3_dataflow_kernels/rope_halfsplit.py`, `programming_examples/llama32_1b/kernel_builder/rope_halfsplit.cc` | Half-split RoPE wrapper. |
| Residual add | `programming_examples/gemma3_dataflow_kernels/residual_add.py` | BF16 vector-add wrapper. |
| GeGLU/down | `programming_examples/gemma3_dataflow_kernels/geglu.py`, `gemma3_stitched_decode.py` | Decode FFN diagnostic path. |

Model-level NPU planning assets:

| Responsibility | Current source |
| --- | --- |
| Real-shape preflight | `gemma3_npu_preflight.py` |
| Per-layer wiring manifest | `gemma3_npu_wiring.py` |
| Static projection weight plan | `gemma3_weight_plan.py` |
| Norm weight plan | `gemma3_norm_weight_plan.py` |
| BO allocation plan | `gemma3_bo_plan.py` |
| Static preload evidence | `gemma3_static_preload.py`, `gemma3_norm_preload.py` |
| Buffer binding plan | `gemma3_buffer_binding.py` |
| Kernel argument binding plan | `gemma3_argument_binding.py` |
| Launch-order manifest | `gemma3_model_runner.py` |
| Stitched decode MLIR | `gemma3_stitched_decode.py`, `gemma3_stitching.py` |
...
Use `gemma3_reproduction_blockers.py` as the machine-readable blocker ledger.
...
python programming_examples/gemma3_herd_sweep/gemma3_quantized_weights.py --self-test
python programming_examples/gemma3_herd_sweep/gemma3_reproduction_blockers.py
python programming_examples/gemma3_herd_sweep/gemma3_paper_compare.py --validate
```
...
```bash
git diff --check
rg -n "^#{1,4} " programming_examples/gemma3_herd_sweep/GEMMA3_MODEL_LOOP_IMPLEMENTATION.md
```
