...
- `gemma3_stitched_decode.py`: active stitched-ELF decode track for real Gemma3
  ...
  KV cache via `--attention-cache-mode hf-prefill`; clean 26-layer evidence is
  recorded in
  `results/gemma3_1b_decode_loop_stitched_hf_prefill_tiled_stats_probe.json`.
  This removes the random/synthetic prefill-cache source from that diagnostic
  path, but it remains non-paper-parity because prefill is not NPU-produced and
  tiled-stat reduction is still host-side.
...
- `gemma3_reproduction_blockers.py` reports Phase F as `BLOCKED` because
  ...
  the measured loop in 2.737617 s, consuming local sliding-window HF cache plus
  the current token for local layers and full 1k HF cache for global layers, and
  validating the 26-layer route with no blockers. That run records 21.929564 s
  loop wall time, 21.491694 s summed NPU `run.start()/wait2()` time across 474
  launches, 0.045601 loop-wall diagnostic TPS, and 0.046530 kernel-only
  diagnostic TPS. It replaces the random synthetic-cache source in the
  diagnostic path, but still leaves NPU-produced prefill KV and NPU-side stat
  reduction open. The
