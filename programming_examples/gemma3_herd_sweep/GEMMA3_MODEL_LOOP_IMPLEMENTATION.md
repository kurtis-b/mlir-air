...
- `gemma3_decode_loop_probe.py`: diagnostic Gemma3 1B staged decode loop probe.
  ...
  wall time and segmented NPU `run.start()/wait2()` time. The default path
  keeps the older single-token FlowQKV attention diagnostic, while explicit
  `--attention-mode tiled-stats-1k` exercises host-batched 1k tiled-stat
...
- `gemma3_reproduction_blockers.py` reports Phase F as `BLOCKED` because
  ...
  a 36.813874 s untimed reference pass, a 35.467162 s measured loop window
  (0.028195 diagnostic TPS), 32.557518 s summed NPU launch time across 2,158
  launches (0.030715 kernel-only diagnostic TPS), 17.118 W full-window package
  power, and a 7.674 W pseudo-NPU package delta. This narrows the attention
  work to production KV-cache construction, NPU-side stat reduction, and
  replacement of the synthetic repeated-cache contract. The
  real 1B blocker report now narrows the launch
  blocker to the narrower set `prefill-1k-npu-not-wired`,
  `production-kv-cache-not-wired`, `npu-attention-reduction-not-wired`,
