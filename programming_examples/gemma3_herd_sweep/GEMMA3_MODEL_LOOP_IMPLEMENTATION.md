...
- `gemma3_stitched_decode.py`: active stitched-ELF decode track for real Gemma3
  ...
  streamed down-projection path still dominates. The decode-loop route also has an
  opt-in stitched post-feedforward mode that reuses the generic `RMSNorm ->
  residual add` stitched ELF for post-feedforward RMSNorm and the final residual;
  26-layer evidence is recorded in
  `results/gemma3_1b_decode_loop_stitched_ingress_attention_o_post_attention_ffn_gate_up_geglu_down_post_feedforward_probe.json`.
  The tiled-stat attention diagnostic can now consume a host HF-produced prefill
  KV cache via `--attention-cache-mode hf-prefill`, seed layer 0 from the HF
  prefill greedy token embedding via `--decode-input-mode hf-prefill-next-token`,
  and run a post-loop host tied-embedding logits diagnostic via
  `--logits-mode host-tied-embedding`; clean 26-layer evidence is recorded in
  `results/gemma3_1b_decode_loop_stitched_hf_prefill_tiled_stats_probe.json`.
  This removes the random/synthetic prefill-cache and random decode-input
  sources from that diagnostic path and records host logits/sampling outside the
  timed NPU window, but it remains non-paper-parity because prefill is not
  NPU-produced, tiled-stat reduction is still host-side, and logits/sampling are
  not NPU-promoted or timed as part of an official paper cell.
...
- Promote the host HF-produced prefill KV cache into an NPU-produced prefill
  cache, move tiled attention-stat reduction onto the NPU, and replace the
  post-loop host logits diagnostic with either NPU logits/sampling or an
  explicitly timed/accounted host sampling path before collecting official
  paper-comparison TTFT/TPS/power numbers.
...
- `gemma3_reproduction_blockers.py` reports Phase F as `BLOCKED` because
  ...
  work to prefill-produced KV-cache construction and NPU-side stat reduction. A
  later clean-provenance decode-loop diagnostic adds
  `--attention-cache-mode hf-prefill`, building the 1k HF prefill cache outside
  the measured loop in 2.711829 s, and `--decode-input-mode
  hf-prefill-next-token`, seeding the loop with the HF prefill greedy token
  embedding (`fill`, token id 6910). Local sliding-window layers consume their
  retained HF cache plus the current token for local layers and full 1k HF cache
  for global layers, and the 26-layer route validates with no blockers. That run
  records 21.582786 s loop wall time, 21.141922 s summed NPU
  `run.start()/wait2()` time across 474 launches, 0.046333 loop-wall diagnostic
  TPS, and 0.047299 kernel-only diagnostic TPS. It also records
  `--logits-mode host-tied-embedding` after the timed NPU window: final RMSNorm
  plus tied embedding argmax takes 1.053632 s on the host, samples token id
  236761 (`.`), and does not match the dense HF one-token decode top-1 token id
  15612 (` cache`). It replaces the random synthetic-cache source and random
  decode input in the diagnostic path, but still leaves NPU-produced prefill KV,
  NPU-side stat reduction, and NPU/timed logits and sampling open. The
  real 1B blocker report now narrows the launch
  blocker to the narrower set `prefill-1k-npu-not-wired`,
  `npu-prefill-kv-cache-not-wired`, `npu-attention-reduction-not-wired`,
  `logits-sampling-host-diagnostic-only`, and
