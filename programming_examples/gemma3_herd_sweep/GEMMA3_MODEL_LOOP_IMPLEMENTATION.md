...
- `gemma3_stitched_decode.py`: active stitched-ELF decode track for real Gemma3
  text inference. The current integrated loop slice is the full decode ingress
  ...
  is recorded in `results/gemma3_1b_stitched_decode_ingress_probe.json`. A
  standalone post-ingress slice, `gemma3_decode_attention_o_projection`, now
  stitches single-token FlowQKV attention into O projection with an aliased
  `1x4x256`/`4x256` attention-output BO; hardware evidence is recorded in
  `results/gemma3_1b_stitched_attention_o_probe.json`, but it is not yet wired
  into the repeated decode loop.
...
- Tune or restructure the stitched ingress route because the 26-layer diagnostic
  ...
  baseline.
- Tune the integrated stitched ingress plus attention/O route because it improves
  loop-wall timing but still trails the staged baseline on kernel-only TPS.
- Stitch the remaining layer tail: post-attention residual, FFN norms/projections,
  GeGLU, down projection, and final residual.
