...
- `gemma3_decode_loop_probe.py`: diagnostic Gemma3 1B decode loop probe.
  ...
  `results/gemma3_1b_decode_loop_stitched_ingress_probe.json`.
...
- Tune or restructure the stitched ingress route because the 26-layer diagnostic
  ...
  baseline.
- Integrate the standalone stitched `attention -> O projection` slice into
  `gemma3_decode_loop_probe.py` with static O-weight BO preload outside timing.
