...
- `gemma3_reproduction_blockers.py` reports Phase F as `BLOCKED` because
  ...
  attention integration, logits/sampling, and the production contiguous
  static-weight BO route remain open. A standalone host-batched tiled-stat
  FlowQKV diagnostic now validates the 1k x 256 attention shape on hardware,
  narrowing the attention work to in-loop reduction and route integration. The
