...
- `gemma3_stitched_decode.py`: active stitched-ELF decode track for real Gemma3
  ...
  recorded in `results/gemma3_1b_stitched_ffn_gate_up_probe.json`. It still
  needs decode-loop integration before it can replace the staged FFN ingress.
...
- Integrate `gemma3_decode_ffn_gate_up` into the repeated decode loop so
  pre-feedforward RMSNorm and the gate/up projections no longer run as one
  staged RMSNorm plus ten staged col-block projection launches per layer.
