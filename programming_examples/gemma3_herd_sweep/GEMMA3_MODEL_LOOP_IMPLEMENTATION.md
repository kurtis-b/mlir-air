...
- `gemma3_reproduction_blockers.py` reports Phase F as `BLOCKED` because
  ...
  for every layer, so `host_fallbacks=[]`. The loop wall window is 7.269038 s,
  or 0.137570 diagnostic TPS, and includes dynamic BO writes, output
  sync/readback, and CPU reference/correlation checks. The summed NPU launch
  windows total 3.744155 s across 1,768 launches, or 0.267083 kernel-only
  diagnostic TPS. Against the paper's 41.1 TPS 1B/1k NPU decode target, the
  post-warmup loop-wall diagnostic is 99.665% low and the kernel-only diagnostic
  is 99.350% low. Full-window direct RAPL reports 17.741 W package power and a
  6.040 W pseudo-NPU package-delta from an 11.701 W quiescent sample; the
  segmented kernel-only pseudo-NPU delta is unusable in this run because its
  quiescent sample was taken while preparation was already busy. These split
  routes are staged correctness and diagnostic timing probes because the current
  full 5-col-block paper module over-allocates tile memory for this shape, and
  because paper 1k KV attention, logits/sampling, diagnostic reference checks,
  and the production contiguous static-weight BO route remain open. The real 1B blocker report now narrows the launch
