...
- `gemma3_stitched_decode.py`: active stitched-ELF decode track for real Gemma3
  ...
  `gemma3_decode_ingress_rms_qkv_qknorm_rope` and the post-ingress
  `gemma3_decode_attention_o_projection` slice. The ingress is an eight-launch
  ...
  is recorded in
  `results/gemma3_1b_decode_loop_stitched_ingress_attention_o_probe.json`. A new
  standalone tail slice, `gemma3_decode_post_attention_residual`, stitches
  post-attention RMSNorm into the attention residual add with an aliased
  `1x1152`/`1152` norm-output BO; clean hardware evidence is recorded in
  `results/gemma3_1b_stitched_post_attention_residual_probe.json`. It still
  needs decode-loop integration before it can replace the staged tail path.
...
- Integrate `gemma3_decode_post_attention_residual` into the repeated decode loop
  so post-attention RMSNorm and the attention residual no longer run as two
  staged launches per layer.
