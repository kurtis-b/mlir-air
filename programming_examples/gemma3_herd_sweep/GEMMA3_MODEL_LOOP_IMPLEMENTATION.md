...
- `gemma3_stitched_decode.py`: active stitched-ELF decode track for real Gemma3
  ...
  and run host tied-embedding logits either after the loop or inside loop-wall
  timing via `--logits-mode host-tied-embedding`; clean 26-layer evidence is
  recorded in
  ...
  sources from that diagnostic path and can record host logits/sampling inside
  the measured loop wall via `--logits-timing included`, but it remains
  non-paper-parity because prefill is not NPU-produced, tiled-stat reduction is
  still host-side, and logits/sampling are not NPU-promoted.
...
- Promote the host HF-produced prefill KV cache into an NPU-produced prefill
  cache and move tiled attention-stat reduction onto the NPU before collecting
  official paper-comparison TTFT/TPS/power numbers. The current host logits path
  can be explicitly timed/accounted, but it is not NPU-promoted.
...
- `gemma3_reproduction_blockers.py` reports Phase F as `BLOCKED` because
  ...
  the measured loop in 2.717121 s, and `--decode-input-mode
  ...
  records 22.705677 s loop wall time, 21.146772 s summed NPU
  `run.start()/wait2()` time across 474 launches, 0.044042 loop-wall diagnostic
  TPS, and 0.047289 kernel-only diagnostic TPS. It also records
  `--logits-mode host-tied-embedding --logits-timing included` inside the timed
  loop wall: final RMSNorm plus tied embedding argmax takes 1.049499 s on the
  host, samples token id 236761 (`.`), and does not match the dense HF one-token
  decode top-1 token id 15612 (` cache`). It replaces the random synthetic-cache
  source and random decode input in the diagnostic path and accounts host logits
  in timing, but still leaves NPU-produced prefill KV and NPU-side stat reduction
  open. The
  real 1B blocker report now narrows the launch
  blocker to the narrower set `prefill-1k-npu-not-wired`,
  `npu-prefill-kv-cache-not-wired`, `npu-attention-reduction-not-wired`, and
