...
- `gemma3_reproduction_blockers.py` reports Phase F as `BLOCKED` because
  ...
  token across all 26 real Gemma3 1B layers with reusable ELF runners. The loop
  wall window is 5.879730 s, or 0.170076 diagnostic TPS, and includes current
  runner BO writes, output sync/readback, and host fallback compute. The summed
  NPU launch windows total 3.641984 s across 1,482 launches, or 0.274576
  kernel-only diagnostic TPS. Against the paper's 41.1 TPS 1B/1k NPU decode
  target, the post-warmup loop-wall diagnostic is 99.586% low and the kernel-only
  diagnostic is 99.332% low. Full-window direct RAPL reports 17.127 W package
  power and a 4.050 W pseudo-NPU package-delta from a 13.077 W quiescent sample;
