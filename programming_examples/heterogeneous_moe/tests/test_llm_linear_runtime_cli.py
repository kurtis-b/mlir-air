...
from llm_linear.manifest import SCHEMA_VERSION, load_json, resolve_package_path
...
def test_linear_runtime_result_schema(moe_dir: Path, tmp_path: Path) -> None:
    ...
    assert result["schema_version"] == SCHEMA_VERSION == "llm-linear-v3"
    assert "performance_proof" not in result
    assert "resident_weights" not in result
    ...
    assert "resident_weight_bytes" not in row
...
def test_linear_runtime_rejects_resident_weights(moe_dir: Path, tmp_path: Path) -> None:
    manifest = _tiny_cpu_manifest(moe_dir, tmp_path)
    manifest["runtime"]["resident_weights"] = True
    with pytest.raises(ValueError, match="resident_weights"):
        LinearRuntime(manifest)
...
def test_linear_direct_runtime_records_native_handoff(
    ...
    direct = result["transfer_summary"]["direct_handoff"]
    assert direct["supported"] is True
    assert direct["contract"] == DIRECT_CONTRACT
    assert direct["mechanism"] == DIRECT_MECHANISM
    assert direct["direct_class"] == DIRECT_CLASS_DEVICE_RESIDENT
    assert direct["zero_host_copy"] is True
    assert direct["probe_report"]["selected_mechanism"] == DIRECT_MECHANISM
    event = result["transfer_events"][0]
    assert event["numpy_host_materializations"] == 0
...
def test_compile_llm_linear_cli(monkeypatch, tmp_path: Path) -> None:
    ...
    with pytest.raises(SystemExit):
        compile_llm_linear.main(["--direct-gpu"])
...
def test_run_llm_linear_suite_cpu_filter_and_direct_failure(tmp_path: Path) -> None:
    ...
    assert summary["schema_version"] == "llm-linear-v3"
    ...
    with pytest.raises(SystemExit):
        run_llm_linear_suite.main(["--resident-weights"])
