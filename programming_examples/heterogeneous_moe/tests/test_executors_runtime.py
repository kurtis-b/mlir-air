...
def test_cpu_executor_kinds_and_stage_container(small_cfg: KernelConfig) -> None:
    ...
    np.testing.assert_allclose(
        CpuExecutor("router", small_cfg.dtype).run(inputs, weights.router),
        reference["logits"],
    )
    np.testing.assert_allclose(
        CpuExecutor("expert", small_cfg.dtype).run(
            reference["expert0_input"], weights.expert0_w1, weights.expert0_w2
        ),
        reference["expert0_output"],
    )
    np.testing.assert_allclose(
        CpuExecutor("aggregation", small_cfg.dtype).run(
            reference["packed_expert_outputs"], reference["weights"]
        ),
        reference["output"],
    )
...
def test_npu_executor_simple_and_cache_paths(
    ...
    assert aggregation.run(
        np.ones((2, 8), dtype=np.float16), np.ones((2, 2), dtype=np.float16)
    ).shape == (2, 4)

    bad = NpuExecutor(
        "bad",
        tmp_path / "bad.air.mlir",
        {},
        tmp_path,
        "npu2",
        small_cfg.dtype,
...
def test_npu_executor_parallel_and_tiled_split(
    monkeypatch, tmp_path: Path, small_cfg: KernelConfig
) -> None:
    inputs = np.ones((2, 4), dtype=np.float16)
    ...
    parallel._split_invokers = {
        "hidden": _filled_invoker(1.0),
        "output": _filled_invoker(2.0),
    }
    ...
    tiled._split_invokers = {
        "hidden": _filled_invoker(1.0),
        "output": _filled_invoker(0.5),
    }
...
def test_gpu_executor_simple_split_and_errors(
    monkeypatch, tmp_path: Path, small_cfg: KernelConfig
) -> None:
    ...
    expert = GpuExecutor(
        ...
        {
            "mode": "parallel_split",
            "hidden": {"artifact": {"so": "hidden.so"}},
            "output": {"artifact": {"so": "output.so"}},
        },
        tmp_path,
        "gfx1150",
        "expert_mlp",
    ...
    expert_out = expert.run(
        inputs, np.ones((4, 8), dtype=np.float16), np.ones((8, 4), dtype=np.float16)
    )
...
def test_moe_runtime_cpu_single_chunk_and_load(
    tmp_path: Path, default_manifest: dict
) -> None:
    manifest_path = tmp_path / "manifest.json"
    manifest_path.write_text(
        __import__("json").dumps(default_manifest), encoding="utf-8"
    )

    with load_runtime(manifest_path) as runtime:
        inputs = random_inputs(runtime.cfg, default_manifest["inputs"]["seed"])
        single = runtime.run(
            inputs, router_mode="top2", validate=True, capture_details=True
        )
        ...
        quick = runtime.run(
            inputs, router_mode="top1", validate=False, capture_details=False
        )
        assert quick["output"].shape == inputs.shape

        longer = np.concatenate([inputs, inputs[:1]], axis=0)
        chunked = runtime.run(
            longer, router_mode="top2", validate=True, capture_details=True
        )
        ...
        quick_chunked = runtime.run(
            longer, router_mode="top2", validate=False, capture_details=False
        )
...
def test_moe_runtime_reports_and_invalid_backend(default_manifest: dict) -> None:
    ...
    bad_manifest = {
        **default_manifest,
        "runtime": {
            **default_manifest["runtime"],
            "stage_backends": dict(default_manifest["runtime"]["stage_backends"]),
        },
    }
    bad_manifest["runtime"]["stage_backends"]["router"] = "bad"
    with pytest.raises(
        ValueError, match="Unsupported source backend|Unsupported backend"
    ):
