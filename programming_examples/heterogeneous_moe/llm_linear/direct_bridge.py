...
def bootstrap_direct_bridge() -> ctypes.CDLL | None:
    global _BOOTSTRAP_DIRECT_BRIDGE_LIB
    if _BOOTSTRAP_DIRECT_BRIDGE_LIB is not None:
        return _BOOTSTRAP_DIRECT_BRIDGE_LIB
...
def reexec_with_direct_bridge_bootstrap(
    script_path: str | Path, argv: list[str]
) -> None:
...
from llm_linear.direct_bridge import bootstrap_direct_bridge

sys._llm_linear_direct_bridge_bootstrap = bootstrap_direct_bridge()
sys.argv = [script] + sys.argv[2:]
runpy.run_path(script, run_name="__main__")
"""
    completed = subprocess.run(
        [sys.executable, "-c", code, str(script_path), *argv],
        env=env,
        check=False,
    )
