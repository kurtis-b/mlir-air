...
* The backend is `LM_GEMV_BACKEND`, which is what this model's decode
  ...
  same 16384-row partitions that compile here at the builder's default
  `m_input` 4 FAIL under Qwen's untiled `_lm_gemv_backend()` with "Repeat
  count exceeds the [0:255] range" (devq 944 against 946), which is why
  Qwen's head needs `m_input` 8 and this one does not. Passing the wrong
  backend would not be a slower gate, it would be a different kernel.
...
from shared.builders.lm_head_gemv_multi import (  # noqa: E402
...
# The decode module compiles the head as `build_lm_head_gemv_module(emb_dim)`
# -- every other argument its default -- so the gate does the same rather than
...
def main():
    ...
    return run_gate(
        cache_dir,
        lambda: build_lm_head_gemv_module(EMB),
        LM_PARTS,
