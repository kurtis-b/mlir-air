...
"""Qwen2.5-0.5B BF16 Inference on MLIR-AIR (NPU2).
...
Usage:
    cd build_peano
    python3 ../qwen25_0_5b_inference.py --compile-only
    python3 ../qwen25_0_5b_inference.py --run-only --n-tokens 32 --prompt "..."
    python3 ../qwen25_0_5b_inference.py --run-only --n-tokens 32 --profile
"""
...
from qwen25_0_5b_decode import (
...
import qwen25_0_5b_decode as _decode_mod
...
def run_npu_prefill(
    ...
    """Run NPU prefill (24 Qwen2.5 layers) and extract KV cache.
...
def run_npu_decode_step(
    ...
    """Run one NPU decode step: 24 blocks + final RMSNorm + LM-head."""
    vocab_size = weights.lm_head.shape[0]
...
MODEL_CHOICES = {"base": "Qwen/Qwen2.5-0.5B", "instruct": "Qwen/Qwen2.5-0.5B-Instruct"}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Qwen2.5-0.5B Inference (NPU)")
    parser.add_argument("--compile-only", action="store_true")
    parser.add_argument("--run-only", action="store_true")
    parser.add_argument("--n-tokens", type=int, default=10)
    parser.add_argument("--profile", action="store_true")
    # Prefill attention defaults to seq-first NPU FlashAttention (head_dim=64 →
    # no hang risk). Pass --cpu-attn to fall back to the host GQA reference.
