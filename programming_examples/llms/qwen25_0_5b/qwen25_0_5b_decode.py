...
"""Qwen2.5-0.5B Decode on MLIR-AIR (NPU2).

Single-token autoregressive generation with KV cache. Mirrors the Phase-2
prefill (qwen25_0_5b_prefill.py) deltas, applied to the M=1 GEMV path:

  1. QKV BIAS fused on-device (Qwen2 family, attention_bias=True; NOT QK-norm —
     that is Qwen3). The fused rms_qkv_bias_rope_gemv ELF adds a per-channel
     bias after the Q/K/V GEMV projection and before RoPE for q/k (HF Qwen2
     order: proj -> +bias -> RoPE(Q,K) -> attention; V is bias-added and used
     directly). bq/bk/bv are passed as static ELF args, so the add happens on
     the NPU together with the RMSNorm + Q/K/V GEMV + RoPE, all in one launch.

  2. Non-aligned dims: emb=896, q_dim=896 (14*64), kv_dim=128 (2*64),
     hidden=4864, head_dim=64. o_proj is SQUARE (q_dim==emb_dim==896).
     The fused llama/qwen3 `o_gemv_ffn` ELF stitches O-GEMV + SwiGLU +
     Down-GEMV via the cascade `matvec_2tile_add` / `matvec_swiglu_rms`
     builders, whose K-chunk requires K % 512 == 0 — which qwen2.5's K=896
     (O proj) and K=4864 (Down) do NOT satisfy. Rather than re-derive cascade
...
Decode attention is CPU (decode_attention_cpu), matching prefill/llama/qwen3.
head_dim=64 → no FA hang risk, but single-token attention is trivial on host.

Phase-1-verified decode GEMV tile configs (see docs phase1_kernels.md):
  Q/O   896×896 : tile_m=8  m_input=8  herd_m=8
  K/V   128×896 : tile_m=8  m_input=8  herd_m=8
  Gate/Up 4864×896 : tile_m=8 m_input=8 herd_m=8
  Down  896×4864 : tile_m=2  m_input=2  herd_m=8   (K=4864 → L2 limit)
"""
...
def _fused_bias_rope_gemv_call(
    ...
    run_decode_block (real x_in + position LUTs). Single owner of the fused arg
    layout. Returns the load_and_run result; output_indices=[14,17,18] ->
    v_b, q_roped, k_roped. The RoPE LUTs (15,16) are position-dependent → NOT
    static (would rotate against a stale BO otherwise).
    """
    emb_dim = config.emb_dim
    n_heads = config.n_heads
    n_kv_heads = config.n_kv_heads
