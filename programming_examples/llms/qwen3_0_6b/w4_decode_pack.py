...
* the `quant_*` schema columns (`quant_contract`), derived from the llama
  packing owner's contract (`_base_contract` below, inlined from the
  study's `llama32_1b_int4.awq_repacker.quant_contract`, study commit
  0d08d195 -- the kernel-side contract IS the same: same packer format,
  same `mv_int4_bf16.cc`) with the provenance fields rewritten to this
  model's reality: RTN fake-quantize of the bf16 checkpoint, no AWQ
  checkpoint.

Quantized here: wo, w_gate, w_up, w_down (the token's largest weight mass;
o_gemv_ffn_int4). NOT quantized -- priced negatives, measured in study
commit 0d08d195: the QKV stage (the 2-launch in-core QK-norm+RoPE epilogue
cannot host in-kernel dequant without a new kernel, and every fallback
launch structure loses more to boundaries than the bytes save) and the LM
head (doc 57 section 5 item 6: -0.46 ms measured ceiling, dequant-bound at
11 GB/s, one-launch form impossible).
"""
...
#: What an UNSET flag means. ON (`w4_decode`): flipped by this PR together
#: with the three-arm verify gate (bf16 A/B, w4 vs the dequant-patched
#: oracle, the quantization bar) -- the study's default (doc 56 H2b, queue
#: item 24; +19-24 % tok/s at study commit 0d08d195), re-landed as a change
#: of THIS constant and nothing else once the gate existed on main.
...
def w4_decode_selected() -> bool:
    ...
    (`W4_DEFAULT`): `w4_decode` is the production default, gated by the
    three-arm verify lit; `QWEN3_W4_DECODE=0` selects the bf16 A/B arm.
...
def _pack(W_q, W_s, W_z):
    ...
    M, K_half = W_q.shape
    K = 2 * K_half
    return pack_inputs(W_q, W_s, W_z, M, K, GROUP_SIZE, M_TILE, K_CHUNK, N_CORES, M)
...
def _base_contract(group_size):
    """The llama packing owner's schema columns, inlined from the study's
    `llama32_1b_int4/awq_repacker.py::quant_contract` (study commit
    0d08d195; the return-dict lines are copied verbatim, minus the two
    contract strings `quant_contract` below overwrites anyway). Main's
    awq_repacker does not carry `quant_contract`, and this module is its
    only consumer on main, so the columns live here rather than growing the
    llama file. Dropped with the import: the llama-loader ast cross-check
    and the kernel-source accfloat probe -- both validate the LLAMA
    loader's defaults, and this model has no AWQ checkpoint or loader
    default to cross-check (`GROUP_SIZE` above is the owner, enforced in
    `quant_contract`)."""
