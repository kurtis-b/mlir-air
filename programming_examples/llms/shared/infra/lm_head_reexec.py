...
def run_gate(cache_dir, build_module, parts, backend, emb, tag="production lm_head_gemv"):
    ...
    cache.compile_and_cache("other_gemv", build_gemv(TINY_M, emb, 8, 4, 8, bfloat16, bfloat16), other_backend)
    cache._save_manifest()
    print(f"[reexec] compiled {tag} (partitions {parts}) + a 1-launch other ELF in "
    ...
    def run_head(x):
        ...
        res = cache.load_and_run("lm_head_gemv", backend, *ins,
    ...
    def run_other(x):
        cache.load_and_run("other_gemv", other_backend, Wt, x, np.zeros(TINY_M, dtype=bfloat16),
    ...
    def judge(label, out, ref, base):
        ...
        bad_parts = sorted({int(np.searchsorted(offs, i, side="right") - 1) for i in bad})
        clean = finite and bad.size == 0 and same
        print(f"[reexec] {label}: {'clean' if clean else 'WRONG'} -- max_rel {rel.max():.2e} "
