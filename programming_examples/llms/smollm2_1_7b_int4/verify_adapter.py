...
The GGUF path comes from `$SMOLLM2_GGUF` (the Makefile sets it), defaulting
to the checkpoint doc 36 recorded.
"""
...
from smollm2_1_7b_int4_weights import load_weights_gguf_q4_0  # noqa: E402
...
DEFAULT_GGUF = "/home/cj/models/gguf/SmolLM2-1.7B-Instruct-Q4_0.gguf"
...
def _gguf_path() -> str:
    return os.environ.get("SMOLLM2_GGUF", DEFAULT_GGUF)
...
def _get_or_load_weights(config):
    key = (_gguf_path(), id(config))
    if key not in _WEIGHTS_CACHE:
        _WEIGHTS_CACHE[key] = load_weights_gguf_q4_0(
            _gguf_path(), HF_MODEL, config=config
        )
