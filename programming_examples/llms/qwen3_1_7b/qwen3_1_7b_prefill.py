...
`build_rms_gemms_rope_module` fusion:
  ...
  2. GQA dims (Qwen3-1.7B): emb_dim=2048, q_dim=n_heads*head_dim=16*128=2048,
     ...
     (shared.builders.o_ffn_multi._build_o_ffn with q_dim=2048, the square
     O form): each GEMM independently resolves drain vs fused-cast per the
     registry. At the production seq (2048) all four resolve fused-cast
     (the prior all-fused `_m64` layout, unchanged); below 2048 the O rows
     resolve drain but the Down shape (seq x 6144 x 2048) has no short-seq
     rows yet, so short-seq mixed builds wait on that registry data.
...
#   registry (o_ffn_gemm_layout). For Qwen3-1.7B q_dim==emb_dim==2048 so the
#   O GEMM is SQUARE; at seq=2048 all four resolve fused-cast (the prior
#   all-fused layout). Replaces the all-fused-cast copy that asserted one
#   _m64 suffix for all four.
# ---------------------------------------------------------------------------
...
def _o_ffn_scratch_plan(seq_len, config):
    ...
    from (qwen3_0_6b_prefill._o_ffn_scratch_plan's pattern; here q_dim ==
    emb_dim so the O GEMM is square), so the host args cannot drift from the
    ELF signature -- and it reads only the registry JSON, so the loaded-cache
    verify path (which skips compile_all_kernels) stays aligned without
...
def _checked_o_ffn_plan(cache, seq_len, config):
    ...
    all-fused seq=2048 layout (short-seq ELFs cannot be built while the Down
    rows are missing, and could not be built pre-rewire) -- anything else is
...
def preload_prefill_weights(weights, config, cache, seq_len, rope_lut_bf16):
    ...
    for layer_idx in range(config.n_layers):
        ...
        # the registry plan (a mixed-method short-seq layout would declare
        # fewer than 4 -- the hardcoded four would overrun the ELF's
        # signature).
