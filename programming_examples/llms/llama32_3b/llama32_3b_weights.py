...
Loads LLAMA-3.2-3B weights from HuggingFace safetensors format and provides
...
@dataclass
class LlamaConfig:
    """LLAMA-3.2-3B model hyperparameters."""
...
def load_weights(
    ...
    Args:
        model_name_or_path: Path to a local directory containing .safetensors
            files, or a HuggingFace model ID like "meta-llama/Llama-3.2-3B".
        dtype: Target numpy dtype for all weight arrays. Default is bfloat16.
        config: Optional LlamaConfig. If None, uses default LLAMA-3.2-3B config.
    ...
    if lm_head_key in key_to_file:
        ...
        # LLAMA-3.2-3B (and other small Llamas) tie lm_head to embed_tokens
