...
def build_gate_module(seq_len, emb_dim, hidden_dim, gate_herd_m=8, gate_herd_n=4):
    ...
    print(
        f"  [gate] Gate GEMM ({g_spec['method']}) {seq_len}x{emb_dim}x{hidden_dim} (tn={g_spec['tile_n']})..."
    )
...
def build_up_module(seq_len, emb_dim, hidden_dim, up_herd_m=8, up_herd_n=4):
    ...
    g_spec = _gemm_spec(seq_len, emb_dim, hidden_dim, "low")  # direct
    print(
        f"  [up] Up GEMM ({g_spec['method']}) {seq_len}x{emb_dim}x{hidden_dim} (tn={g_spec['tile_n']})..."
    )
    ...
    slices = [
        KernelSlice(up_ir, "ug", {0: 0, 1: 1, 2: 2}, extern_syms=_gemm_externs(g_spec))
    ]
...
def compile_all_kernels(cache, config, seq_len, verbose=False, cpu_attn=False):
    ...
    cache.compile_and_cache(
        "gate", build_gate_module(seq_len, emb_dim, hidden_dim), _gate_backend(verbose)
    )

    print("\n--- up (Up GEMM, direct) ---")
    cache.compile_and_cache(
        "up", build_up_module(seq_len, emb_dim, hidden_dim), _up_backend(verbose)
    )
