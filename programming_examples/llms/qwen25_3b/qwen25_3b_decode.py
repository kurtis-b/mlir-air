...
"""Qwen2.5-3B Decode on MLIR-AIR (NPU2).

Single-token autoregressive generation with KV cache. Mirrors the Phase-2
prefill (qwen25_3b_prefill.py) deltas, applied to the M=1 GEMV path:

  1. QKV BIAS fused on-device (Qwen2 family, attention_bias=True; NOT QK-norm —
     that is Qwen3). The rms_qkv_bias_rope_gemv ELF adds the per-channel bias
     (bq/bk/bv passed as static ELF args, indices 9/10/11) after the Q/K/V GEMV
     and BEFORE RoPE for q/k (HF Qwen2 order: proj -> +bias -> RoPE(Q,K) ->
     attention; V is bias-added and used directly). RMSNorm + Q/K/V GEMV +
     bias-add + RoPE(Q,K) are all fused into that one ELF.

  2. Dims: emb=2048, q_dim=2048 (16*128), kv_dim=256 (2*128),
     hidden=11008, head_dim=128. o_proj is SQUARE (q_dim==emb_dim==2048).
     The fused llama/qwen3 `o_gemv_ffn` ELF stitches O-GEMV + SwiGLU +
     Down-GEMV via the cascade `matvec_2tile_add` / `matvec_swiglu_rms`
     builders, whose K-chunk requires K % 512 == 0 — which qwen2.5's K=11008
...
Phase-1-verified decode GEMV tile configs (see docs phase1_kernels.md):
  Q/O   2048×2048 : tile_m=8  m_input=8  herd_m=8
  K/V   256×2048  : tile_m=8  m_input=8  herd_m=8
  Gate/Up 11008×2048 : tile_m=8 m_input=8 herd_m=8
  Down  2048×11008 : tile_m=2  m_input=1  herd_m=8  (NPU standalone ELF; needs
        omit_pingpong=all + its own down_mv.o)

O / Gate / Up / Down ALL run on NPU as standalone GEMV ELFs. The residual add /
FFN RMSNorm / SwiGLU stay on host (single-token, cheap, exact). Down (K=11008)
compiles + runs on NPU once ping-pong is disabled.

MEASURED (2026-06-27): with O/Gate/Up/Down on NPU, the residual + FFN RMSNorm +
SwiGLU left on host are ~0.045ms/layer of M=1 numpy (microbench at emb=2048/
hidden=11008). Moving them to a standalone NPU ELF would add a ~0.7-1.4ms
dispatch per op to replace ~0.02ms numpy — a large regression — and folding into
the GEMV epilogue is the matvec_swiglu_rms cascade that overflows L1 here. Glue
stays on host by design; the decode cost is now the NPU GEMVs (real compute).
"""
...
_GEMV_QO = (8, 8, 8)  # 2048×2048 (Q proj, O proj)
_GEMV_KV = (8, 8, 8)  # 256×2048 (K proj, V proj)
_GEMV_GATEUP = (8, 8, 8)  # 11008×2048 (Gate proj, Up proj)
_GEMV_DOWN = (2, 1, 8)  # 2048×11008 (Down proj; K=11008 → L1-bound m_input=1)

# Down GEMV runs on NPU as a standalone matvec ELF. The standalone builder stages
# A in L2 (not L1) and streams B L3→L1, so K=11008 compiles + runs (verified
# standalone: M=2048 K=11008 tile_m=2/m_input=1/herd_m=8, bit-identical to f32
# ref). Only the *fused* cascade (matvec_swiglu_rms, which double-buffers the full
# K in L1) overflows 64 KB L1 — so the FFN glue stays on host by design.
...
def _gemv_backend(verbose=False, name="gemv", omit_pingpong=False):
    ...
    # Large-K GEMV (Down, K=11008): the B vector is 22 KB; ping-pong would
    # double-buffer it (4×22 KB = 88 KB > 64 KB L1) and AIE allocation fails.
    # Disable ping-pong so the standalone Down ELF fits L1 (same reason OGF
    # turns ping-pong off for its K=8192 down). K=2048 GEMVs keep ping-pong.
    if omit_pingpong:
...
def compile_decode_kernels(cache, config, verbose=False):
    ...
    # Down proj GEMV (M=emb=2048, K=hidden=11008). The STANDALONE matvec stages
    # A in L2 and streams B L3→L1, so K=11008 compiles + runs (unlike the fused
    # cascade, which double-buffers full-K in L1 and overflows). tile_m=2 m_input=1.
    # Down needs tile_m=2 (K=11008 L2 budget), but the shared mv.o is built with
    # DIM_M_OUTPUT=8 for o/gate/up. linalg_fill_bf16 zeros DIM_M_OUTPUT rows, so
    # down must link its own mv.o (DIM_M_OUTPUT=tile_m=2) → down_mv.o.
...
def run_decode_block(
    ...
    # Down projection: M=emb=2048, K=hidden=11008. The STANDALONE matvec builder
    # stages A in L2 and streams B L3→L1, so K=11008 compiles + runs (verified
    # bit-identical to f32 ref). Runs on NPU as its own standalone down_gemv ELF.
