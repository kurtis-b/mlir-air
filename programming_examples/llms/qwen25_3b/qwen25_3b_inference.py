...
Usage:
    cd build_peano
    python3 ../qwen25_3b_inference.py --compile-only
    python3 ../qwen25_3b_inference.py --run-only --n-tokens 32 --prompt "..."
    python3 ../qwen25_3b_inference.py --run-only --n-tokens 32 --profile
"""
...
import qwen25_3b_decode as _decode_mod
...
def _preload_decode_weights(decode_cache, weights, config):
    ...
    for li in range(config.n_layers):
        ...
        # down_gemv: M=emb=2048, K=hidden=11008. Standalone GEMV ELF on NPU
...
def run_npu_prefill(
    ...
    """Run NPU prefill (36 Qwen2.5 layers) and extract KV cache.
...
MODEL_CHOICES = {"base": "Qwen/Qwen2.5-3B", "instruct": "Qwen/Qwen2.5-3B-Instruct"}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Qwen2.5-3B Inference (NPU)")
