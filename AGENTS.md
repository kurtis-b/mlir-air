...
This repository keeps human-facing build, run, test, and AIR semantics documentation in `docs/`. The `agents/` directory is an operational overlay for agents: routing, state checks, incremental rebuild policy, and focused verification choices.
...
For task routing and helper commands, see `agents/README.md`.
