...
def build():
    ...
    l3a = MemRefType.get([M, K], xrt); l3b = MemRefType.get([K, N], xrt); l3c = MemRefType.get([M, N], xrt)
    ...
    mm.attributes["link_with"] = StringAttr.get("mm.o"); mm.attributes["llvm.emit_c_interface"] = UnitAttr.get()
            ...
            def seg(sa, sb, sc):
                ...
                def hb(tx, ty, sx, sy, ha, hb_, hc):
                    for k in range_(0, K, TILE_K):
                        b1 = AllocOp(ta, [], []); b2 = AllocOp(tb, [], []); b3 = AllocOp(tc, [], [])
                        dma_memcpy_nd(b1, ha, src_offsets=[0, k], src_sizes=[M, TILE_K], src_strides=[K, 1])
                        dma_memcpy_nd(b2, hb_, src_offsets=[k, 0], src_sizes=[TILE_K, N], src_strides=[N, 1])
                        dma_memcpy_nd(b3, hc, src_offsets=[0, 0], src_sizes=[M, N], src_strides=[N, 1])
                        CallOp(mm, [b1, b2, b3])
                        dma_memcpy_nd(hc, b3, dst_offsets=[0, 0], dst_sizes=[M, N], dst_strides=[N, 1])
                        DeallocOp(b1); DeallocOp(b2); DeallocOp(b3)
                        yield_([])
                hb.attributes["link_with"] = StringAttr.get("mm.o")

w = tempfile.mkdtemp(prefix="accum-aircc-"); os.chdir(w)
mod = build()
be = XRTBackend(verbose=False, omit_while_true_loop=False, output_format="elf",
                instance_name="acc", debug_ir=True)
try:
    be.compile(mod); print("compiled (unexpected -- no kernel object)")
...
def in_loop(text):
    ...
    for l in lines[i:]:
        ...
        if depth <= 0 and len(body) > 1: break
...
for d in dumps:
    ...
    if before is not None:
        print(f"    data-movement ops inside the loop: {in_loop(before)} -> {in_loop(after)}")
