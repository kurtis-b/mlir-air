...
PIPELINE = (
    "air-dependency,air-hoist-dma-in-accum-pattern,air-broadcast-detection,"
...
def report(name, mod):
    ...
    if text is None:
        print(
            f"  {name:22s} lowering FAILED: {err.splitlines()[0][:90] if err else '?'}"
        )
        return
    packet = text.count('"npu_dma_packet"')
    chans = text.count("air.channel @")
    print(
        f"  {name:22s} channel decls={chans:3d}   packet-typed={packet:3d}   "
...
def main():
    ...
    report(
        "addnorm (3 streams)",
        build_addnorm_module(104, COLS, herd_x=HERD_X, pre_add=True),
    )

    print("\nthe streamed builders `fused` uses instead, over all 4096 rows:")
    try:
        report(
            "layer_norm (1 stream)", build_layer_norm_module(ROWS, COLS, herd_x=HERD_X)
        )
    except Exception as e:  # noqa: BLE001
        print(f"  layer_norm build failed: {type(e).__name__}: {str(e)[:120]}")
    try:
        report(
            "eltwise_add (2 streams)",
