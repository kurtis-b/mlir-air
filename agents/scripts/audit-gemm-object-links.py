...
WHY THIS EXISTS
    ...
    `mm_*.o` filename of every external GEMM, and eleven shipped LLM deployments
    resolve their tile configurations through it. The cross-deployment regression
    rule says any change there re-runs `make verify` on all eleven -- hours of
    ...
    The study's Phase E1 made both names functions of (tile_m, tile_n); the R5a
    port keeps the method-era names as an alias at tile_n=128, so
    `gemm_variant_names` mints bare `_m32` / `mm_m32.o` there and `_m32n64` /
    `mm_m32n64.o` only for genuinely non-128 variants. A bare name below is
    therefore read as a DIM_N=128 claim, never a parse error. This script found
    that `qwen25_0_5b` had been mis-linked all along: it resolves its O and Down
    GEMMs at the registry's tile_n=32, overrides tile_n to 128 because the narrow
    tile is numerically broken for the drain path, and under the old method-only
    naming still asked for `mm_m32.o` -- an object compiled at DIM_N=128. Correct
    by accident.

USAGE
    python3 agents/scripts/audit-gemm-object-links.py            # every model
...
# Every model dir whose verify_adapter exposes the `compile_prefill_kernels`
# seam this audit drives. The GGUF/q4nx family, qwen25_3b_q4 and smolvla ship
# no such seam (their adapters compile inside the runner), so they are not
# auditable this way and are deliberately absent.
MODELS = [
    "llama32_1b",
    "llama32_1b_int4",
    "llama32_3b",
